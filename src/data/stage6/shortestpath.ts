...
const H = MIN_HEAP_SOURCE; // prepended to Dijkstra solutions so a heap is available
...
const drafts: ProblemDraft[] = [
  /* ---- drills (easy → hard) ---- */
  {
    ...
    patternIds: P,
    statement:
      "Given a directed graph on `n` nodes with non-negative weighted edges `[u, v, w]`, return the shortest distance from `src` to `dst`, or -1 if `dst` is unreachable.",
    examples: [
      { input: "5, [[0,1,4],[0,2,1],[2,1,2],[1,3,1],[2,3,5]], 0, 3", output: "4" },
      { input: "3, [[0,1,1],[1,2,1]], 0, 2", output: "2" },
      { input: "3, [[0,1,1]], 0, 2", output: "-1" },
    ],
    constraints: ["1 <= n <= 10000", "0 <= w"],
    ...
    visible: [
      { args: [5, [[0, 1, 4], [0, 2, 1], [2, 1, 2], [1, 3, 1], [2, 3, 5]], 0, 3], expected: 4 },
      { args: [3, [[0, 1, 1], [1, 2, 1]], 0, 2], expected: 2 },
      { args: [3, [[0, 1, 1]], 0, 2], expected: -1 },
    ],
    hidden: [
      { args: [1, [], 0, 0], expected: 0 },
      { args: [4, [[0, 1, 1], [1, 2, 1], [2, 3, 1]], 0, 3], expected: 3 },
      { args: [4, [[0, 1, 10], [0, 2, 3], [2, 1, 4], [1, 3, 2], [2, 3, 8]], 0, 3], expected: 9 },
      { args: [2, [[0, 1, 7]], 0, 1], expected: 7 },
      { args: [5, [[0, 1, 2], [0, 2, 4], [1, 2, 1], [2, 4, 3], [1, 3, 7], [3, 4, 1]], 0, 4], expected: 6 },
      { args: [3, [[0, 1, 1]], 0, 2], expected: -1 },
    ],
    hints: [
...
`,
  ...
  },
  {
    ...
    patternIds: P,
    statement:
      "Given a directed graph on `n` nodes with non-negative weighted edges `[u, v, w]` and a source, return an array of the shortest distances from `src` to every node (use -1 for unreachable nodes).",
    examples: [
      { input: "5, [[0,1,4],[0,2,1],[2,1,2],[1,3,1],[2,3,5]], 0", output: "[0,3,1,4,-1]" },
      { input: "3, [[0,1,1],[1,2,1]], 0", output: "[0,1,2]" },
      { input: "3, [[0,1,1]], 0", output: "[0,1,-1]" },
    ],
    constraints: ["1 <= n <= 10000", "0 <= w"],
    ...
    visible: [
      { args: [5, [[0, 1, 4], [0, 2, 1], [2, 1, 2], [1, 3, 1], [2, 3, 5]], 0], expected: [0, 3, 1, 4, -1] },
      { args: [3, [[0, 1, 1], [1, 2, 1]], 0], expected: [0, 1, 2] },
      { args: [3, [[0, 1, 1]], 0], expected: [0, 1, -1] },
    ],
    hidden: [
      { args: [1, [], 0], expected: [0] },
      { args: [4, [[0, 1, 1], [1, 2, 1], [2, 3, 1]], 0], expected: [0, 1, 2, 3] },
      { args: [2, [[0, 1, 7]], 0], expected: [0, 7] },
      { args: [4, [[1, 0, 2], [1, 2, 3]], 1], expected: [2, 0, 3, -1] },
      { args: [3, [[0, 1, 5], [0, 2, 2], [2, 1, 1]], 0], expected: [0, 3, 2] },
      { args: [3, [[0, 1, 1], [1, 2, 1]], 0], expected: [0, 1, 2] },
    ],
    hints: [
...
`,
  ...
  },
  {
    ...
    patternIds: P,
    statement:
      "A signal starts at node `src` and travels along directed weighted edges `[u, v, w]` (w = travel time). Return the time for all `n` nodes to receive the signal, or -1 if some node never does.",
    examples: [
      { input: "4, [[0,1,1],[0,2,1],[1,3,1],[2,3,1]], 0", output: "2" },
      { input: "3, [[0,1,1],[1,2,1]], 0", output: "2" },
      { input: "3, [[0,1,1]], 0", output: "-1" },
    ],
    constraints: ["1 <= n <= 10000", "0 <= w"],
    ...
    visible: [
      { args: [4, [[0, 1, 1], [0, 2, 1], [1, 3, 1], [2, 3, 1]], 0], expected: 2 },
      { args: [3, [[0, 1, 1], [1, 2, 1]], 0], expected: 2 },
      { args: [3, [[0, 1, 1]], 0], expected: -1 },
    ],
    hidden: [
      { args: [1, [], 0], expected: 0 },
      { args: [2, [[0, 1, 5]], 0], expected: 5 },
      { args: [4, [[0, 1, 2], [0, 2, 1], [2, 3, 4]], 0], expected: 5 },
      { args: [3, [[0, 1, 3], [0, 2, 1], [2, 1, 1]], 0], expected: 2 },
      { args: [5, [[0, 1, 1], [1, 2, 1], [2, 3, 1], [3, 4, 1]], 0], expected: 4 },
      { args: [3, [[0, 1, 1]], 0], expected: -1 },
    ],
    hints: [
...
`,
  ...
  },
  {
    ...
    patternIds: P,
    statement:
      "Given `n` cities and directed weighted flights `[u, v, price]`, return the cheapest price from `src` to `dst` using at most `K` stops (i.e. at most K+1 flights), or -1 if there is no such route.",
    examples: [
      { input: "4, [[0,1,100],[1,2,100],[2,0,100],[1,3,600],[2,3,200]], 0, 3, 1", output: "700" },
      { input: "3, [[0,1,100],[1,2,100],[0,2,500]], 0, 2, 1", output: "200" },
      { input: "3, [[0,1,100],[1,2,100],[0,2,500]], 0, 2, 0", output: "500" },
    ],
    constraints: ["1 <= n <= 1000", "0 <= K < n", "0 <= price"],
    ...
    hidden: [
      { args: [4, [[0, 1, 1], [1, 2, 1], [2, 3, 1]], 0, 3, 3], expected: 3 },
      { args: [4, [[0, 1, 1], [1, 2, 1], [2, 3, 1]], 0, 3, 1], expected: -1 },
      { args: [2, [[0, 1, 50]], 0, 1, 0], expected: 50 },
      { args: [5, [[0, 1, 5], [1, 2, 5], [0, 2, 100], [2, 3, 5], [0, 4, 3]], 0, 3, 2], expected: 15 },
      { args: [3, [[0, 1, 2], [1, 2, 2]], 0, 2, 5], expected: 4 },
      { args: [3, [[0, 1, 100], [1, 2, 100], [0, 2, 500]], 0, 2, 1], expected: 200 },
    ],
    hints: [
  ...
  },
  {
    ...
    patternIds: P,
    statement:
      "Given a directed graph on `n` nodes with weighted edges `[u, v, w]` that may be negative (but with no negative cycle) and a source, return the shortest distance from `src` to every node (use -1 for unreachable).",
    examples: [
      { input: "5, [[0,1,4],[0,2,1],[2,1,2],[1,3,1],[2,3,5]], 0", output: "[0,3,1,4,-1]" },
      { input: "3, [[0,1,-1],[1,2,-2]], 0", output: "[0,-1,-3]" },
      { input: "3, [[0,1,4],[0,2,5],[1,2,-3]], 0", output: "[0,4,1]" },
    ],
    constraints: ["1 <= n <= 2000", "no negative cycle"],
    ...
    visible: [
      { args: [5, [[0, 1, 4], [0, 2, 1], [2, 1, 2], [1, 3, 1], [2, 3, 5]], 0], expected: [0, 3, 1, 4, -1] },
      { args: [3, [[0, 1, -1], [1, 2, -2]], 0], expected: [0, -1, -3] },
      { args: [3, [[0, 1, 4], [0, 2, 5], [1, 2, -3]], 0], expected: [0, 4, 1] },
    ],
    hidden: [
      { args: [1, [], 0], expected: [0] },
      { args: [4, [[0, 1, 1], [1, 2, -1], [2, 3, 2]], 0], expected: [0, 1, 0, 2] },
      { args: [2, [[0, 1, -5]], 0], expected: [0, -5] },
      { args: [4, [[0, 1, 1], [1, 2, 2]], 0], expected: [0, 1, 3, -1] },
      { args: [3, [[0, 1, 3], [1, 2, -2], [0, 2, 5]], 0], expected: [0, 3, 1] },
      { args: [3, [[0, 1, -1], [1, 2, -2]], 0], expected: [0, -1, -3] },
    ],
    hints: [
  ...
  },
  {
    ...
    patternIds: P,
    statement:
      "Given a directed weighted graph on `n` nodes with edges `[u, v, w]` (weights may be negative), return whether it contains a negative-weight cycle.",
    examples: [
      { input: "3, [[0,1,1],[1,2,-1],[2,0,-1]]", output: "true" },
      { input: "3, [[0,1,1],[1,2,2]]", output: "false" },
      { input: "2, [[0,1,-1],[1,0,-1]]", output: "true" },
    ],
    ...
    visible: [
      { args: [3, [[0, 1, 1], [1, 2, -1], [2, 0, -1]]], expected: true },
      { args: [3, [[0, 1, 1], [1, 2, 2]]], expected: false },
      { args: [2, [[0, 1, -1], [1, 0, -1]]], expected: true },
    ],
    hidden: [
      { args: [4, [[0, 1, 1], [1, 2, 1], [2, 3, 1]]], expected: false },
      { args: [3, [[0, 1, -2], [1, 2, -2], [2, 0, 3]]], expected: true },
      { args: [1, []], expected: false },
      { args: [4, [[0, 1, 1], [1, 2, -3], [2, 1, 1]]], expected: true },
      { args: [3, [[0, 1, 2], [1, 2, 3], [2, 0, -4]]], expected: false },
      { args: [3, [[0, 1, 1], [1, 2, 2]]], expected: false },
    ],
    hints: [
  ...
  /* ---- held-out test pool ---- */
  {
    ...
    patternIds: P,
    statement:
      "Given a directed weighted graph (non-negative edges `[u, v, w]`), a source, and a distance `limit`, return how many nodes (including the source) have shortest distance from `src` at most `limit`.",
    examples: [
      { input: "5, [[0,1,4],[0,2,1],[2,1,2],[1,3,1],[2,3,5]], 0, 4", output: "4" },
      { input: "3, [[0,1,1],[1,2,1]], 0, 1", output: "2" },
      { input: "3, [[0,1,1]], 0, 5", output: "2" },
    ],
    constraints: ["1 <= n <= 10000", "0 <= w", "0 <= limit"],
    ...
    visible: [
      { args: [5, [[0, 1, 4], [0, 2, 1], [2, 1, 2], [1, 3, 1], [2, 3, 5]], 0, 4], expected: 4 },
      { args: [3, [[0, 1, 1], [1, 2, 1]], 0, 1], expected: 2 },
      { args: [3, [[0, 1, 1]], 0, 5], expected: 2 },
    ],
    hidden: [
      { args: [1, [], 0, 0], expected: 1 },
      { args: [4, [[0, 1, 1], [1, 2, 1], [2, 3, 1]], 0, 2], expected: 3 },
      { args: [2, [[0, 1, 7]], 0, 5], expected: 1 },
      { args: [4, [[0, 1, 2], [0, 2, 3]], 0, 3], expected: 3 },
      { args: [5, [[0, 1, 1], [1, 2, 1], [2, 3, 1], [3, 4, 1]], 0, 2], expected: 3 },
      { args: [3, [[0, 1, 1], [1, 2, 1]], 0, 1], expected: 2 },
    ],
    hints: [
...
`,
  ...
  },
  {
    ...
    patternIds: P,
    statement:
      "Given a directed graph with positive weighted edges `[u, v, w]`, return the number of distinct shortest paths from `src` to `dst` (0 if unreachable).",
    examples: [
      { input: "4, [[0,1,1],[0,2,1],[1,3,1],[2,3,1]], 0, 3", output: "2" },
      { input: "3, [[0,1,1],[1,2,1]], 0, 2", output: "1" },
      { input: "3, [[0,1,1]], 0, 2", output: "0" },
    ],
    constraints: ["1 <= n <= 2000", "1 <= w"],
    ...
    visible: [
      { args: [4, [[0, 1, 1], [0, 2, 1], [1, 3, 1], [2, 3, 1]], 0, 3], expected: 2 },
      { args: [3, [[0, 1, 1], [1, 2, 1]], 0, 2], expected: 1 },
      { args: [3, [[0, 1, 1]], 0, 2], expected: 0 },
    ],
    hidden: [
      { args: [1, [], 0, 0], expected: 1 },
      { args: [5, [[0, 1, 1], [0, 2, 1], [1, 3, 1], [2, 3, 1], [3, 4, 1]], 0, 4], expected: 2 },
      { args: [4, [[0, 1, 1], [0, 2, 2], [1, 3, 2], [2, 3, 1]], 0, 3], expected: 2 },
      { args: [2, [[0, 1, 3]], 0, 1], expected: 1 },
      { args: [6, [[0, 1, 1], [0, 2, 1], [1, 3, 1], [2, 3, 1], [3, 4, 1], [3, 5, 1]], 0, 5], expected: 2 },
      { args: [3, [[0, 1, 1], [1, 2, 1]], 0, 2], expected: 1 },
    ],
    hints: [
...
`,
  ...
  },
  {
    ...
    patternIds: P,
    statement:
      "Given a directed weighted graph (edges `[u, v, w]`, weights may be negative but no negative cycle) and nodes `a` and `b`, return the shortest distance from `a` to `b`, or -1 if `b` is unreachable, using the all-pairs Floyd-Warshall method.",
    examples: [
      { input: "5, [[0,1,4],[0,2,1],[2,1,2],[1,3,1],[2,3,5]], 0, 3", output: "4" },
      { input: "3, [[0,1,1],[1,2,1]], 0, 2", output: "2" },
      { input: "3, [[0,1,1]], 0, 2", output: "-1" },
    ...
    hidden: [
      { args: [1, [], 0, 0], expected: 0 },
      { args: [4, [[0, 1, 1], [1, 2, 1], [2, 3, 1]], 3, 0], expected: -1 },
      { args: [2, [[0, 1, 7]], 0, 1], expected: 7 },
      { args: [4, [[0, 1, 5], [1, 2, 2], [0, 2, 9]], 0, 2], expected: 7 },
      { args: [3, [[0, 1, 3], [1, 2, -2], [0, 2, 5]], 0, 2], expected: 1 },
      { args: [3, [[0, 1, 1], [1, 2, 1]], 0, 2], expected: 2 },
    ],
    hints: [
  ...
  },
  {
    ...
    patternIds: P,
    statement:
      "Given `n` cities and UNDIRECTED weighted roads `[u, v, w]`, and a `threshold`, return the city that can reach the fewest other cities within total distance `threshold`. If several tie, return the one with the largest index.",
    examples: [
      { input: "4, [[0,1,3],[1,2,1],[1,3,4],[2,3,1]], 4", output: "3" },
      { input: "5, [[0,1,2],[0,4,8],[1,2,3],[1,4,2],[2,3,1],[3,4,1]], 2", output: "0" },
      { input: "3, [[0,1,1],[1,2,1]], 2", output: "2" },
    ],
    constraints: ["1 <= n <= 400", "roads are undirected"],
    ...
    visible: [
      ...
      { args: [3, [[0, 1, 1], [1, 2, 1]], 2], expected: 2 },
    ],
    hidden: [
      { args: [1, [], 5], expected: 0 },
      { args: [2, [[0, 1, 10]], 5], expected: 1 },
      { args: [4, [[0, 1, 1], [1, 2, 1], [2, 3, 1]], 1], expected: 3 },
      { args: [3, [[0, 1, 2], [1, 2, 2], [0, 2, 3]], 3], expected: 2 },
      { args: [5, [[0, 1, 1], [1, 2, 1], [2, 3, 1], [3, 4, 1]], 2], expected: 4 },
      { args: [3, [[0, 1, 1], [1, 2, 1]], 2], expected: 2 },
    ],
    hints: [
...
export const shortestPathProblems: Problem[] = drafts.map((d) => mkProblem(S, d));

export const shortestPathMcqs: QuizQuestion[] = [
  {
    id: "s6-sp-dijkstra",
    kind: "mcq",
    ...
    answerIndex: 1,
    explanation:
      "A negative edge can improve an already-settled node, breaking Dijkstra's greedy invariant — use Bellman-Ford instead.",
  },
  {
    id: "s6-sp-bellman",
    kind: "mcq",
    ...
    answerIndex: 1,
    explanation:
      "Shortest paths use at most n − 1 edges; a further improvement can only come from cycling through a negative-weight loop.",
  },
];

export const shortestPathModule: Module = {
  id: "m-pat-shortest-path",
  stageId: S,
  title: "Shortest Paths (Dijkstra & Bellman-Ford)",
  kind: "patternModule",
  summary:
    "Weighted shortest paths — Dijkstra with a heap for non-negative graphs, Bellman-Ford for negative edges and cycle detection, Floyd-Warshall for all pairs.",
  lessonSections: [
    {
...
**Pitfalls:** running Dijkstra on negative edges; forgetting to **skip stale heap entries** (the \`d > dist[u]\` guard); in bounded Bellman-Ford, relaxing off the *current* round instead of a **snapshot** (which lets a path exceed the stop limit); mixing up **directed vs undirected** (add both directions for undirected roads); and integer overflow in the heap encoding — keep \`BASE\` safely above the node count. Every drill ships a heap/relaxation solution plus an alternative — compare them and work easy to hard.`,
  ...
  complexityQuestionIds: ["s6-sp-dijkstra", "s6-sp-bellman"],
  badgeId: "badge-pat-shortest-path",
  prerequisiteModuleIds: ["m-pat-union-find"],
};
