    ...
    solutions: [
      {
        ...
        ts: "function maxProfit1(prices: number[]): number {\n  let hold = -Infinity, sold = 0;\n  for (const p of prices) {\n    hold = Math.max(hold, -p);\n    sold = Math.max(sold, hold + p);\n  }\n  return sold;\n}\n",
        commentedCode: {
          js: "function maxProfit1(prices) {\n  // hold is the best balance after one buy; sold is the best completed profit.\n  let hold = -Infinity, sold = 0;\n\n  // Treat each price as the possible day to buy or sell.\n  for (const p of prices) {\n    // Either keep the earlier buy or buy once today for a balance of -p.\n    hold = Math.max(hold, -p);\n    // Either keep the best sale or sell the share represented by hold today.\n    sold = Math.max(sold, hold + p);\n  }\n\n  // No trade leaves sold at zero, so unprofitable inputs are handled naturally.\n  return sold;\n}\n",
          ts: "function maxProfit1(prices: number[]): number {\n  // hold is the best balance after one buy; sold is the best completed profit.\n  let hold = -Infinity, sold = 0;\n\n  // Treat each price as the possible day to buy or sell.\n  for (const p of prices) {\n    // Either keep the earlier buy or buy once today for a balance of -p.\n    hold = Math.max(hold, -p);\n    // Either keep the best sale or sell the share represented by hold today.\n    sold = Math.max(sold, hold + p);\n  }\n\n  // No trade leaves sold at zero, so unprofitable inputs are handled naturally.\n  return sold;\n}\n",
        },
        time: "O(n)",
        space: "O(1)",
      },
      {
        ...
        commentedCode: {
          js: "function maxProfit1(prices) {\n  const n = prices.length;\n  // Cache each (day, holding-state) result to avoid repeated suffix work.\n  const memo = new Map();\n\n  // Return the best future profit starting on day i in the given state.\n  const go = (i, holding) => {\n    // Once every day is processed, no more profit can be earned.\n    if (i >= n) return 0;\n    // Two holding states per day produce a unique numeric cache key.\n    const key = i * 2 + holding;\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // If holding, either wait or sell today; selling ends the one allowed trade.\n    if (holding) v = Math.max(go(i + 1, 1), prices[i]);\n    // If free, either skip today or pay today's price and enter the holding state.\n    else v = Math.max(go(i + 1, 0), -prices[i] + go(i + 1, 1));\n\n    // Save the optimal choice for this state before returning it.\n    memo.set(key, v);\n    return v;\n  };\n\n  // Begin before any purchase has been made.\n  return go(0, 0);\n}\n",
          ts: "function maxProfit1(prices: number[]): number {\n  const n = prices.length;\n  // Cache each (day, holding-state) result to avoid repeated suffix work.\n  const memo = new Map<number, number>();\n\n  // Return the best future profit starting on day i in the given state.\n  const go = (i: number, holding: number): number => {\n    // Once every day is processed, no more profit can be earned.\n    if (i >= n) return 0;\n    // Two holding states per day produce a unique numeric cache key.\n    const key = i * 2 + holding;\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // If holding, either wait or sell today; selling ends the one allowed trade.\n    if (holding) v = Math.max(go(i + 1, 1), prices[i]);\n    // If free, either skip today or pay today's price and enter the holding state.\n    else v = Math.max(go(i + 1, 0), -prices[i] + go(i + 1, 1));\n\n    // Save the optimal choice for this state before returning it.\n    memo.set(key, v);\n    return v;\n  };\n\n  // Begin before any purchase has been made.\n  return go(0, 0);\n}\n",
        },
        time: "O(n)",
        space: "O(n)",
    ...
    solutions: [
      {
        ...
        ts: "function maxProfit2(prices: number[]): number {\n  let total = 0;\n  for (let i = 1; i < prices.length; i++)\n    if (prices[i] > prices[i - 1]) total += prices[i] - prices[i - 1];\n  return total;\n}\n",
        commentedCode: {
          js: "function maxProfit2(prices) {\n  // Accumulate the profit from every profitable one-day rise.\n  let total = 0;\n  // Compare each day with the immediately preceding day.\n  for (let i = 1; i < prices.length; i++)\n    // A positive step can be captured by buying before it and selling after it.\n    if (prices[i] > prices[i - 1]) total += prices[i] - prices[i - 1];\n  // Adjacent rises combine into the same profit as holding across a longer climb.\n  return total;\n}\n",
          ts: "function maxProfit2(prices: number[]): number {\n  // Accumulate the profit from every profitable one-day rise.\n  let total = 0;\n  // Compare each day with the immediately preceding day.\n  for (let i = 1; i < prices.length; i++)\n    // A positive step can be captured by buying before it and selling after it.\n    if (prices[i] > prices[i - 1]) total += prices[i] - prices[i - 1];\n  // Adjacent rises combine into the same profit as holding across a longer climb.\n  return total;\n}\n",
        },
        time: "O(n)",
        space: "O(1)",
      },
      {
        ...
        ts: "function maxProfit2(prices: number[]): number {\n  const n = prices.length;\n  const memo = new Map<number, number>();\n  const go = (i: number, holding: number): number => {\n    if (i >= n) return 0;\n    const key = i * 2 + holding;\n    if (memo.has(key)) return memo.get(key);\n    let v;\n    if (holding) v = Math.max(go(i + 1, 1), prices[i] + go(i + 1, 0));\n    else v = Math.max(go(i + 1, 0), -prices[i] + go(i + 1, 1));\n    memo.set(key, v);\n    return v;\n  };\n  return go(0, 0);\n}\n",
        commentedCode: {
          js: "function maxProfit2(prices) {\n  const n = prices.length;\n  // Memoize the best suffix profit for each day and ownership state.\n  const memo = new Map();\n\n  const go = (i, holding) => {\n    // There are no actions left after the final day.\n    if (i >= n) return 0;\n    const key = i * 2 + holding;\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // While holding, either keep the share or sell it and become free tomorrow.\n    if (holding) v = Math.max(go(i + 1, 1), prices[i] + go(i + 1, 0));\n    // While free, either wait or buy today and hold a share tomorrow.\n    else v = Math.max(go(i + 1, 0), -prices[i] + go(i + 1, 1));\n\n    memo.set(key, v);\n    return v;\n  };\n\n  // Start on day zero with no share; repeated re-entry enables unlimited trades.\n  return go(0, 0);\n}\n",
          ts: "function maxProfit2(prices: number[]): number {\n  const n = prices.length;\n  // Memoize the best suffix profit for each day and ownership state.\n  const memo = new Map<number, number>();\n\n  const go = (i: number, holding: number): number => {\n    // There are no actions left after the final day.\n    if (i >= n) return 0;\n    const key = i * 2 + holding;\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // While holding, either keep the share or sell it and become free tomorrow.\n    if (holding) v = Math.max(go(i + 1, 1), prices[i] + go(i + 1, 0));\n    // While free, either wait or buy today and hold a share tomorrow.\n    else v = Math.max(go(i + 1, 0), -prices[i] + go(i + 1, 1));\n\n    memo.set(key, v);\n    return v;\n  };\n\n  // Start on day zero with no share; repeated re-entry enables unlimited trades.\n  return go(0, 0);\n}\n",
        },
        time: "O(n)",
        space: "O(n)",
    ...
    solutions: [
      {
        ...
        ts: "function maxProfitCooldown(prices: number[]): number {\n  if (prices.length === 0) return 0;\n  let hold = -Infinity, sold = 0, free = 0;\n  for (const p of prices) {\n    const ph = hold, ps = sold, pf = free;\n    hold = Math.max(ph, pf - p);\n    sold = ph + p;\n    free = Math.max(pf, ps);\n  }\n  return Math.max(sold, free);\n}\n",
        commentedCode: {
          js: "function maxProfitCooldown(prices) {\n  // An empty schedule offers no transaction.\n  if (prices.length === 0) return 0;\n  // hold owns a share, sold sold today, and free may buy today.\n  let hold = -Infinity, sold = 0, free = 0;\n\n  for (const p of prices) {\n    // Snapshot yesterday's states so every transition advances exactly one day.\n    const ph = hold, ps = sold, pf = free;\n    // Keep holding, or buy only from yesterday's cooldown-free state.\n    hold = Math.max(ph, pf - p);\n    // Selling today must come from a share held yesterday.\n    sold = ph + p;\n    // Staying free or completing yesterday's cooldown both permit rest today.\n    free = Math.max(pf, ps);\n  }\n\n  // A valid final answer owns no share, so finish sold or free.\n  return Math.max(sold, free);\n}\n",
          ts: "function maxProfitCooldown(prices: number[]): number {\n  // An empty schedule offers no transaction.\n  if (prices.length === 0) return 0;\n  // hold owns a share, sold sold today, and free may buy today.\n  let hold = -Infinity, sold = 0, free = 0;\n\n  for (const p of prices) {\n    // Snapshot yesterday's states so every transition advances exactly one day.\n    const ph = hold, ps = sold, pf = free;\n    // Keep holding, or buy only from yesterday's cooldown-free state.\n    hold = Math.max(ph, pf - p);\n    // Selling today must come from a share held yesterday.\n    sold = ph + p;\n    // Staying free or completing yesterday's cooldown both permit rest today.\n    free = Math.max(pf, ps);\n  }\n\n  // A valid final answer owns no share, so finish sold or free.\n  return Math.max(sold, free);\n}\n",
        },
        time: "O(n)",
        space: "O(1)",
      },
      {
        ...
        ts: "function maxProfitCooldown(prices: number[]): number {\n  const n = prices.length;\n  const memo = new Map<number, number>();\n  const go = (i: number, holding: number): number => {\n    if (i >= n) return 0;\n    const key = i * 2 + holding;\n    if (memo.has(key)) return memo.get(key);\n    let v;\n    if (holding) v = Math.max(go(i + 1, 1), prices[i] + go(i + 2, 0));\n    else v = Math.max(go(i + 1, 0), -prices[i] + go(i + 1, 1));\n    memo.set(key, v);\n    return v;\n  };\n  return go(0, 0);\n}\n",
        commentedCode: {
          js: "function maxProfitCooldown(prices) {\n  const n = prices.length;\n  // Cache each day/ownership state because different decisions reach it repeatedly.\n  const memo = new Map();\n\n  const go = (i, holding) => {\n    // Passing the final day means there is no remaining profit.\n    if (i >= n) return 0;\n    const key = i * 2 + holding;\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // If holding, wait one day or sell and skip the next day for the cooldown.\n    if (holding) v = Math.max(go(i + 1, 1), prices[i] + go(i + 2, 0));\n    // If free, wait or buy now and carry the share into tomorrow.\n    else v = Math.max(go(i + 1, 0), -prices[i] + go(i + 1, 1));\n\n    memo.set(key, v);\n    return v;\n  };\n\n  // Initially we are free to buy on the first day.\n  return go(0, 0);\n}\n",
          ts: "function maxProfitCooldown(prices: number[]): number {\n  const n = prices.length;\n  // Cache each day/ownership state because different decisions reach it repeatedly.\n  const memo = new Map<number, number>();\n\n  const go = (i: number, holding: number): number => {\n    // Passing the final day means there is no remaining profit.\n    if (i >= n) return 0;\n    const key = i * 2 + holding;\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // If holding, wait one day or sell and skip the next day for the cooldown.\n    if (holding) v = Math.max(go(i + 1, 1), prices[i] + go(i + 2, 0));\n    // If free, wait or buy now and carry the share into tomorrow.\n    else v = Math.max(go(i + 1, 0), -prices[i] + go(i + 1, 1));\n\n    memo.set(key, v);\n    return v;\n  };\n\n  // Initially we are free to buy on the first day.\n  return go(0, 0);\n}\n",
        },
        time: "O(n)",
        space: "O(n)",
    ...
    solutions: [
      {
        ...
        ts: "function maxProfitFee(prices: number[], fee: number): number {\n  let hold = -Infinity, cash = 0;\n  for (const p of prices) {\n    const ph = hold;\n    hold = Math.max(ph, cash - p);\n    cash = Math.max(cash, ph + p - fee);\n  }\n  return cash;\n}\n",
        commentedCode: {
          js: "function maxProfitFee(prices, fee) {\n  // hold is the best balance owning one share; cash owns none.\n  let hold = -Infinity, cash = 0;\n\n  for (const p of prices) {\n    // Preserve yesterday's holding value for a legitimate sell transition.\n    const ph = hold;\n    // Either keep the existing share or buy today using current cash.\n    hold = Math.max(ph, cash - p);\n    // Either remain in cash or sell yesterday's share and pay the fee once.\n    cash = Math.max(cash, ph + p - fee);\n  }\n\n  // Completed profit must end in the no-share cash state.\n  return cash;\n}\n",
          ts: "function maxProfitFee(prices: number[], fee: number): number {\n  // hold is the best balance owning one share; cash owns none.\n  let hold = -Infinity, cash = 0;\n\n  for (const p of prices) {\n    // Preserve yesterday's holding value for a legitimate sell transition.\n    const ph = hold;\n    // Either keep the existing share or buy today using current cash.\n    hold = Math.max(ph, cash - p);\n    // Either remain in cash or sell yesterday's share and pay the fee once.\n    cash = Math.max(cash, ph + p - fee);\n  }\n\n  // Completed profit must end in the no-share cash state.\n  return cash;\n}\n",
        },
        time: "O(n)",
        space: "O(1)",
      },
      {
        ...
        ts: "function maxProfitFee(prices: number[], fee: number): number {\n  const n = prices.length;\n  const memo = new Map<number, number>();\n  const go = (i: number, holding: number): number => {\n    if (i >= n) return 0;\n    const key = i * 2 + holding;\n    if (memo.has(key)) return memo.get(key);\n    let v;\n    if (holding) v = Math.max(go(i + 1, 1), prices[i] - fee + go(i + 1, 0));\n    else v = Math.max(go(i + 1, 0), -prices[i] + go(i + 1, 1));\n    memo.set(key, v);\n    return v;\n  };\n  return go(0, 0);\n}\n",
        commentedCode: {
          js: "function maxProfitFee(prices, fee) {\n  const n = prices.length;\n  // Store one answer for every day and ownership state.\n  const memo = new Map();\n\n  const go = (i, holding) => {\n    // No days remain, so no additional completed profit is possible.\n    if (i >= n) return 0;\n    const key = i * 2 + holding;\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // If holding, either wait or sell today, charging the fee on that sale.\n    if (holding) v = Math.max(go(i + 1, 1), prices[i] - fee + go(i + 1, 0));\n    // If free, either wait or buy today and enter the holding state.\n    else v = Math.max(go(i + 1, 0), -prices[i] + go(i + 1, 1));\n\n    memo.set(key, v);\n    return v;\n  };\n\n  // Start with no stock and no accumulated profit.\n  return go(0, 0);\n}\n",
          ts: "function maxProfitFee(prices: number[], fee: number): number {\n  const n = prices.length;\n  // Store one answer for every day and ownership state.\n  const memo = new Map<number, number>();\n\n  const go = (i: number, holding: number): number => {\n    // No days remain, so no additional completed profit is possible.\n    if (i >= n) return 0;\n    const key = i * 2 + holding;\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // If holding, either wait or sell today, charging the fee on that sale.\n    if (holding) v = Math.max(go(i + 1, 1), prices[i] - fee + go(i + 1, 0));\n    // If free, either wait or buy today and enter the holding state.\n    else v = Math.max(go(i + 1, 0), -prices[i] + go(i + 1, 1));\n\n    memo.set(key, v);\n    return v;\n  };\n\n  // Start with no stock and no accumulated profit.\n  return go(0, 0);\n}\n",
        },
        time: "O(n)",
        space: "O(n)",
    ...
    solutions: [
      {
        ...
        ts: "function paintHouse(costs: number[][]): number {\n  if (costs.length === 0) return 0;\n  let a = costs[0][0], b = costs[0][1], c = costs[0][2];\n  for (let i = 1; i < costs.length; i++) {\n    const na = costs[i][0] + Math.min(b, c);\n    const nb = costs[i][1] + Math.min(a, c);\n    const nc = costs[i][2] + Math.min(a, b);\n    a = na; b = nb; c = nc;\n  }\n  return Math.min(a, b, c);\n}\n",
        commentedCode: {
          js: "function paintHouse(costs) {\n  // No houses require no painting cost.\n  if (costs.length === 0) return 0;\n  // Base row: total cost when the first house ends in each of three colors.\n  let a = costs[0][0], b = costs[0][1], c = costs[0][2];\n\n  for (let i = 1; i < costs.length; i++) {\n    // Color 0 may follow only a previous house painted color 1 or 2.\n    const na = costs[i][0] + Math.min(b, c);\n    // Color 1 may follow only color 0 or 2.\n    const nb = costs[i][1] + Math.min(a, c);\n    // Color 2 may follow only color 0 or 1.\n    const nc = costs[i][2] + Math.min(a, b);\n    // Advance all three states together after computing from the old row.\n    a = na; b = nb; c = nc;\n  }\n\n  // The final house may use whichever ending color is cheapest.\n  return Math.min(a, b, c);\n}\n",
          ts: "function paintHouse(costs: number[][]): number {\n  // No houses require no painting cost.\n  if (costs.length === 0) return 0;\n  // Base row: total cost when the first house ends in each of three colors.\n  let a = costs[0][0], b = costs[0][1], c = costs[0][2];\n\n  for (let i = 1; i < costs.length; i++) {\n    // Color 0 may follow only a previous house painted color 1 or 2.\n    const na = costs[i][0] + Math.min(b, c);\n    // Color 1 may follow only color 0 or 2.\n    const nb = costs[i][1] + Math.min(a, c);\n    // Color 2 may follow only color 0 or 1.\n    const nc = costs[i][2] + Math.min(a, b);\n    // Advance all three states together after computing from the old row.\n    a = na; b = nb; c = nc;\n  }\n\n  // The final house may use whichever ending color is cheapest.\n  return Math.min(a, b, c);\n}\n",
        },
        time: "O(n)",
        space: "O(1)",
      },
      {
        ...
        commentedCode: {
          js: "function paintHouse(costs) {\n  const n = costs.length;\n  // An empty street has zero cost.\n  if (n === 0) return 0;\n  // Cache each (house index, previous color) subproblem.\n  const memo = new Map();\n\n  const go = (i, prev) => {\n    // Every house has been painted successfully.\n    if (i === n) return 0;\n    // Four previous-color values include sentinel 3 for no previous house.\n    const key = i * 4 + prev;\n    if (memo.has(key)) return memo.get(key);\n\n    let best = Infinity;\n    // Try each of the three colors for the current house.\n    for (let c = 0; c < 3; c++)\n      // Exclude the adjacent duplicate, then add this cost to the best suffix.\n      if (c !== prev) best = Math.min(best, costs[i][c] + go(i + 1, c));\n\n    memo.set(key, best);\n    return best;\n  };\n\n  // Sentinel 3 means the first house is free to choose any color.\n  return go(0, 3);\n}\n",
          ts: "function paintHouse(costs: number[][]): number {\n  const n = costs.length;\n  // An empty street has zero cost.\n  if (n === 0) return 0;\n  // Cache each (house index, previous color) subproblem.\n  const memo = new Map<number, number>();\n\n  const go = (i: number, prev: number): number => {\n    // Every house has been painted successfully.\n    if (i === n) return 0;\n    // Four previous-color values include sentinel 3 for no previous house.\n    const key = i * 4 + prev;\n    if (memo.has(key)) return memo.get(key);\n\n    let best = Infinity;\n    // Try each of the three colors for the current house.\n    for (let c = 0; c < 3; c++)\n      // Exclude the adjacent duplicate, then add this cost to the best suffix.\n      if (c !== prev) best = Math.min(best, costs[i][c] + go(i + 1, c));\n\n    memo.set(key, best);\n    return best;\n  };\n\n  // Sentinel 3 means the first house is free to choose any color.\n  return go(0, 3);\n}\n",
        },
        time: "O(n)",
        space: "O(n)",
    ...
    solutions: [
      {
        ...
        ts: "function maxProfitK(k: number, prices: number[]): number {\n  const n = prices.length;\n  if (n === 0 || k === 0) return 0;\n  const buy = new Array(k + 1).fill(-Infinity);\n  const sell = new Array(k + 1).fill(0);\n  for (const p of prices) {\n    for (let j = 1; j <= k; j++) {\n      buy[j] = Math.max(buy[j], sell[j - 1] - p);\n      sell[j] = Math.max(sell[j], buy[j] + p);\n    }\n  }\n  return sell[k];\n}\n",
        commentedCode: {
          js: "function maxProfitK(k, prices) {\n  const n = prices.length;\n  // With no day or no allowed transaction, profit must be zero.\n  if (n === 0 || k === 0) return 0;\n  // buy[j] is the best balance after transaction j's buy; impossible initially.\n  const buy = new Array(k + 1).fill(-Infinity);\n  // sell[j] is the best balance after at most j completed sales.\n  const sell = new Array(k + 1).fill(0);\n\n  // Feed each market price through every transaction layer.\n  for (const p of prices) {\n    for (let j = 1; j <= k; j++) {\n      // Keep the old jth buy, or fund a new one with j-1 completed sales.\n      buy[j] = Math.max(buy[j], sell[j - 1] - p);\n      // Keep the old jth sale, or close the currently best jth position.\n      sell[j] = Math.max(sell[j], buy[j] + p);\n    }\n  }\n\n  // This layer permits every transaction up to the requested limit.\n  return sell[k];\n}\n",
          ts: "function maxProfitK(k: number, prices: number[]): number {\n  const n = prices.length;\n  // With no day or no allowed transaction, profit must be zero.\n  if (n === 0 || k === 0) return 0;\n  // buy[j] is the best balance after transaction j's buy; impossible initially.\n  const buy = new Array(k + 1).fill(-Infinity);\n  // sell[j] is the best balance after at most j completed sales.\n  const sell = new Array(k + 1).fill(0);\n\n  // Feed each market price through every transaction layer.\n  for (const p of prices) {\n    for (let j = 1; j <= k; j++) {\n      // Keep the old jth buy, or fund a new one with j-1 completed sales.\n      buy[j] = Math.max(buy[j], sell[j - 1] - p);\n      // Keep the old jth sale, or close the currently best jth position.\n      sell[j] = Math.max(sell[j], buy[j] + p);\n    }\n  }\n\n  // This layer permits every transaction up to the requested limit.\n  return sell[k];\n}\n",
        },
        time: "O(n·k)",
        space: "O(k)",
      },
      {
        ...
        commentedCode: {
          js: "function maxProfitK(k, prices) {\n  const n = prices.length;\n  // Cache every combination of day, ownership, and remaining sales.\n  const memo = new Map();\n\n  const go = (i, holding, left) => {\n    // No remaining day or sale capacity means no further completed profit.\n    if (i >= n || left === 0) return 0;\n    // Flatten the three state dimensions into a unique cache key.\n    const key = (i * (k + 1) + left) * 2 + holding;\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // While holding, wait or sell today and consume one transaction.\n    if (holding) v = Math.max(go(i + 1, 1, left), prices[i] + go(i + 1, 0, left - 1));\n    // While free, wait or buy; buying does not complete a transaction yet.\n    else v = Math.max(go(i + 1, 0, left), -prices[i] + go(i + 1, 1, left));\n\n    memo.set(key, v);\n    return v;\n  };\n\n  // Start free on day zero with all k sales available.\n  return go(0, 0, k);\n}\n",
          ts: "function maxProfitK(k: number, prices: number[]): number {\n  const n = prices.length;\n  // Cache every combination of day, ownership, and remaining sales.\n  const memo = new Map<number, number>();\n\n  const go = (i: number, holding: number, left: number): number => {\n    // No remaining day or sale capacity means no further completed profit.\n    if (i >= n || left === 0) return 0;\n    // Flatten the three state dimensions into a unique cache key.\n    const key = (i * (k + 1) + left) * 2 + holding;\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // While holding, wait or sell today and consume one transaction.\n    if (holding) v = Math.max(go(i + 1, 1, left), prices[i] + go(i + 1, 0, left - 1));\n    // While free, wait or buy; buying does not complete a transaction yet.\n    else v = Math.max(go(i + 1, 0, left), -prices[i] + go(i + 1, 1, left));\n\n    memo.set(key, v);\n    return v;\n  };\n\n  // Start free on day zero with all k sales available.\n  return go(0, 0, k);\n}\n",
        },
        time: "O(n·k)",
        space: "O(n·k)",
    ...
    solutions: [
      {
        ...
        ts: "function maxProfit3(prices: number[]): number {\n  let buy1 = -Infinity, sell1 = 0, buy2 = -Infinity, sell2 = 0;\n  for (const p of prices) {\n    buy1 = Math.max(buy1, -p);\n    sell1 = Math.max(sell1, buy1 + p);\n    buy2 = Math.max(buy2, sell1 - p);\n    sell2 = Math.max(sell2, buy2 + p);\n  }\n  return sell2;\n}\n",
        commentedCode: {
          js: "function maxProfit3(prices) {\n  // These are the best balances after each action in two transactions.\n  let buy1 = -Infinity, sell1 = 0, buy2 = -Infinity, sell2 = 0;\n\n  for (const p of prices) {\n    // First buy: keep the old position or buy at today's price.\n    buy1 = Math.max(buy1, -p);\n    // First sale: keep its profit or close the first position today.\n    sell1 = Math.max(sell1, buy1 + p);\n    // Second buy: use the first sale's profit to offset today's purchase.\n    buy2 = Math.max(buy2, sell1 - p);\n    // Second sale: keep its profit or close the second position today.\n    sell2 = Math.max(sell2, buy2 + p);\n  }\n\n  // sell2 also represents using fewer than two trades because it starts at zero.\n  return sell2;\n}\n",
          ts: "function maxProfit3(prices: number[]): number {\n  // These are the best balances after each action in two transactions.\n  let buy1 = -Infinity, sell1 = 0, buy2 = -Infinity, sell2 = 0;\n\n  for (const p of prices) {\n    // First buy: keep the old position or buy at today's price.\n    buy1 = Math.max(buy1, -p);\n    // First sale: keep its profit or close the first position today.\n    sell1 = Math.max(sell1, buy1 + p);\n    // Second buy: use the first sale's profit to offset today's purchase.\n    buy2 = Math.max(buy2, sell1 - p);\n    // Second sale: keep its profit or close the second position today.\n    sell2 = Math.max(sell2, buy2 + p);\n  }\n\n  // sell2 also represents using fewer than two trades because it starts at zero.\n  return sell2;\n}\n",
        },
        time: "O(n)",
        space: "O(1)",
      },
      {
        ...
        ts: "function maxProfit3(prices: number[]): number {\n  const n = prices.length, k = 2;\n  const memo = new Map<number, number>();\n  const go = (i: number, holding: number, left: number): number => {\n    if (i >= n || left === 0) return 0;\n    const key = (i * (k + 1) + left) * 2 + holding;\n    if (memo.has(key)) return memo.get(key);\n    let v;\n    if (holding) v = Math.max(go(i + 1, 1, left), prices[i] + go(i + 1, 0, left - 1));\n    else v = Math.max(go(i + 1, 0, left), -prices[i] + go(i + 1, 1, left));\n    memo.set(key, v);\n    return v;\n  };\n  return go(0, 0, k);\n}\n",
        commentedCode: {
          js: "function maxProfit3(prices) {\n  // This is the bounded-transaction state machine with exactly two sale slots.\n  const n = prices.length, k = 2;\n  const memo = new Map();\n\n  const go = (i, holding, left) => {\n    // Stop when days or remaining completed transactions run out.\n    if (i >= n || left === 0) return 0;\n    // Encode day, remaining sales, and ownership in one key.\n    const key = (i * (k + 1) + left) * 2 + holding;\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // If holding, either wait or sell and spend one of the two sale slots.\n    if (holding) v = Math.max(go(i + 1, 1, left), prices[i] + go(i + 1, 0, left - 1));\n    // If free, either wait or buy without consuming a transaction yet.\n    else v = Math.max(go(i + 1, 0, left), -prices[i] + go(i + 1, 1, left));\n\n    memo.set(key, v);\n    return v;\n  };\n\n  // Begin with no stock and both completed transactions available.\n  return go(0, 0, k);\n}\n",
          ts: "function maxProfit3(prices: number[]): number {\n  // This is the bounded-transaction state machine with exactly two sale slots.\n  const n = prices.length, k = 2;\n  const memo = new Map<number, number>();\n\n  const go = (i: number, holding: number, left: number): number => {\n    // Stop when days or remaining completed transactions run out.\n    if (i >= n || left === 0) return 0;\n    // Encode day, remaining sales, and ownership in one key.\n    const key = (i * (k + 1) + left) * 2 + holding;\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // If holding, either wait or sell and spend one of the two sale slots.\n    if (holding) v = Math.max(go(i + 1, 1, left), prices[i] + go(i + 1, 0, left - 1));\n    // If free, either wait or buy without consuming a transaction yet.\n    else v = Math.max(go(i + 1, 0, left), -prices[i] + go(i + 1, 1, left));\n\n    memo.set(key, v);\n    return v;\n  };\n\n  // Begin with no stock and both completed transactions available.\n  return go(0, 0, k);\n}\n",
        },
        time: "O(n)",
        space: "O(n)",
    ...
    solutions: [
      {
        ...
        commentedCode: {
          js: "function matrixChainOrder(dims) {\n  // One fewer matrix exists than dimension entries.\n  const n = dims.length - 1;\n  // Zero or one matrix needs no multiplication.\n  if (n <= 1) return 0;\n  // Cache the minimum cost for every inclusive matrix interval [i, j].\n  const memo = new Map();\n\n  const go = (i, j) => {\n    // A single matrix is already a product and costs zero.\n    if (i === j) return 0;\n    const key = i * n + j;\n    if (memo.has(key)) return memo.get(key);\n\n    let best = Infinity;\n    // Split after k, solve both subchains, then multiply their result matrices.\n    for (let k = i; k < j; k++)\n      best = Math.min(best, go(i, k) + go(k + 1, j) + dims[i] * dims[k + 1] * dims[j + 1]);\n\n    memo.set(key, best);\n    return best;\n  };\n\n  // Optimize the parenthesization of the complete matrix chain.\n  return go(0, n - 1);\n}\n",
          ts: "function matrixChainOrder(dims: number[]): number {\n  // One fewer matrix exists than dimension entries.\n  const n = dims.length - 1;\n  // Zero or one matrix needs no multiplication.\n  if (n <= 1) return 0;\n  // Cache the minimum cost for every inclusive matrix interval [i, j].\n  const memo = new Map<number, number>();\n\n  const go = (i: number, j: number): number => {\n    // A single matrix is already a product and costs zero.\n    if (i === j) return 0;\n    const key = i * n + j;\n    if (memo.has(key)) return memo.get(key);\n\n    let best = Infinity;\n    // Split after k, solve both subchains, then multiply their result matrices.\n    for (let k = i; k < j; k++)\n      best = Math.min(best, go(i, k) + go(k + 1, j) + dims[i] * dims[k + 1] * dims[j + 1]);\n\n    memo.set(key, best);\n    return best;\n  };\n\n  // Optimize the parenthesization of the complete matrix chain.\n  return go(0, n - 1);\n}\n",
        },
        time: "O(n³)",
        space: "O(n²)",
      },
      {
        ...
        ts: "function matrixChainOrder(dims: number[]): number {\n  const n = dims.length - 1;\n  if (n <= 1) return 0;\n  const dp = Array.from({ length: n }, () => new Array(n).fill(0));\n  for (let len = 2; len <= n; len++)\n    for (let i = 0; i + len - 1 < n; i++) {\n      const j = i + len - 1;\n      dp[i][j] = Infinity;\n      for (let k = i; k < j; k++)\n        dp[i][j] = Math.min(dp[i][j], dp[i][k] + dp[k + 1][j] + dims[i] * dims[k + 1] * dims[j + 1]);\n    }\n  return dp[0][n - 1];\n}\n",
        commentedCode: {
          js: "function matrixChainOrder(dims) {\n  // Matrix i has dimensions dims[i] by dims[i + 1].\n  const n = dims.length - 1;\n  // A chain with at most one matrix needs no scalar multiplication.\n  if (n <= 1) return 0;\n  // dp[i][j] stores the cheapest product of inclusive matrices i through j.\n  const dp = Array.from({ length: n }, () => new Array(n).fill(0));\n\n  // Build longer chains only after all shorter subchains are available.\n  for (let len = 2; len <= n; len++)\n    for (let i = 0; i + len - 1 < n; i++) {\n      const j = i + len - 1;\n      // Any real split will improve this initial sentinel.\n      dp[i][j] = Infinity;\n      // Try making the final multiplication between k and k + 1.\n      for (let k = i; k < j; k++)\n        dp[i][j] = Math.min(dp[i][j], dp[i][k] + dp[k + 1][j] + dims[i] * dims[k + 1] * dims[j + 1]);\n    }\n\n  // The corner interval covers every matrix in the chain.\n  return dp[0][n - 1];\n}\n",
          ts: "function matrixChainOrder(dims: number[]): number {\n  // Matrix i has dimensions dims[i] by dims[i + 1].\n  const n = dims.length - 1;\n  // A chain with at most one matrix needs no scalar multiplication.\n  if (n <= 1) return 0;\n  // dp[i][j] stores the cheapest product of inclusive matrices i through j.\n  const dp = Array.from({ length: n }, () => new Array(n).fill(0));\n\n  // Build longer chains only after all shorter subchains are available.\n  for (let len = 2; len <= n; len++)\n    for (let i = 0; i + len - 1 < n; i++) {\n      const j = i + len - 1;\n      // Any real split will improve this initial sentinel.\n      dp[i][j] = Infinity;\n      // Try making the final multiplication between k and k + 1.\n      for (let k = i; k < j; k++)\n        dp[i][j] = Math.min(dp[i][j], dp[i][k] + dp[k + 1][j] + dims[i] * dims[k + 1] * dims[j + 1]);\n    }\n\n  // The corner interval covers every matrix in the chain.\n  return dp[0][n - 1];\n}\n",
        },
        time: "O(n³)",
        space: "O(n²)",
    ...
    solutions: [
      {
        ...
        ts: "function burstBalloons(nums: number[]): number {\n  const b = [1, ...nums, 1];\n  const n = b.length;\n  const memo = new Map<number, number>();\n  const go = (i: number, j: number): number => {\n    if (j - i < 2) return 0;\n    const key = i * n + j;\n    if (memo.has(key)) return memo.get(key);\n    let best = 0;\n    for (let k = i + 1; k < j; k++)\n      best = Math.max(best, go(i, k) + go(k, j) + b[i] * b[k] * b[j]);\n    memo.set(key, best);\n    return best;\n  };\n  return go(0, n - 1);\n}\n",
        commentedCode: {
          js: "function burstBalloons(nums) {\n  // Sentinel balloons make every real interval have fixed boundary values.\n  const b = [1, ...nums, 1];\n  const n = b.length;\n  // Cache the best score inside each open interval (i, j).\n  const memo = new Map();\n\n  const go = (i, j) => {\n    // Adjacent boundaries contain no real balloon to burst.\n    if (j - i < 2) return 0;\n    const key = i * n + j;\n    if (memo.has(key)) return memo.get(key);\n\n    let best = 0;\n    // Choose k as the last balloon burst inside this interval.\n    for (let k = i + 1; k < j; k++)\n      // By bursting k last, i and j are its known surviving neighbors.\n      best = Math.max(best, go(i, k) + go(k, j) + b[i] * b[k] * b[j]);\n\n    memo.set(key, best);\n    return best;\n  };\n\n  // Solve the open interval containing all original balloons.\n  return go(0, n - 1);\n}\n",
          ts: "function burstBalloons(nums: number[]): number {\n  // Sentinel balloons make every real interval have fixed boundary values.\n  const b = [1, ...nums, 1];\n  const n = b.length;\n  // Cache the best score inside each open interval (i, j).\n  const memo = new Map<number, number>();\n\n  const go = (i: number, j: number): number => {\n    // Adjacent boundaries contain no real balloon to burst.\n    if (j - i < 2) return 0;\n    const key = i * n + j;\n    if (memo.has(key)) return memo.get(key);\n\n    let best = 0;\n    // Choose k as the last balloon burst inside this interval.\n    for (let k = i + 1; k < j; k++)\n      // By bursting k last, i and j are its known surviving neighbors.\n      best = Math.max(best, go(i, k) + go(k, j) + b[i] * b[k] * b[j]);\n\n    memo.set(key, best);\n    return best;\n  };\n\n  // Solve the open interval containing all original balloons.\n  return go(0, n - 1);\n}\n",
        },
        time: "O(n³)",
        space: "O(n²)",
      },
      {
        ...
        ts: "function burstBalloons(nums: number[]): number {\n  const b = [1, ...nums, 1];\n  const n = b.length;\n  const dp = Array.from({ length: n }, () => new Array(n).fill(0));\n  for (let len = 2; len < n; len++)\n    for (let i = 0; i + len < n; i++) {\n      const j = i + len;\n      for (let k = i + 1; k < j; k++)\n        dp[i][j] = Math.max(dp[i][j], dp[i][k] + dp[k][j] + b[i] * b[k] * b[j]);\n    }\n  return dp[0][n - 1];\n}\n",
        commentedCode: {
          js: "function burstBalloons(nums) {\n  // Add virtual value-1 balloons at both ends as permanent boundaries.\n  const b = [1, ...nums, 1];\n  const n = b.length;\n  // dp[i][j] is the maximum score for balloons strictly between i and j.\n  const dp = Array.from({ length: n }, () => new Array(n).fill(0));\n\n  // Increase boundary distance so both smaller open intervals are ready first.\n  for (let len = 2; len < n; len++)\n    for (let i = 0; i + len < n; i++) {\n      const j = i + len;\n      // Try each interior balloon k as the final one removed from (i, j).\n      for (let k = i + 1; k < j; k++)\n        // Combine both independent sides with k's fixed-boundary final burst.\n        dp[i][j] = Math.max(dp[i][j], dp[i][k] + dp[k][j] + b[i] * b[k] * b[j]);\n    }\n\n  // The widest open interval contains every original balloon.\n  return dp[0][n - 1];\n}\n",
          ts: "function burstBalloons(nums: number[]): number {\n  // Add virtual value-1 balloons at both ends as permanent boundaries.\n  const b = [1, ...nums, 1];\n  const n = b.length;\n  // dp[i][j] is the maximum score for balloons strictly between i and j.\n  const dp = Array.from({ length: n }, () => new Array(n).fill(0));\n\n  // Increase boundary distance so both smaller open intervals are ready first.\n  for (let len = 2; len < n; len++)\n    for (let i = 0; i + len < n; i++) {\n      const j = i + len;\n      // Try each interior balloon k as the final one removed from (i, j).\n      for (let k = i + 1; k < j; k++)\n        // Combine both independent sides with k's fixed-boundary final burst.\n        dp[i][j] = Math.max(dp[i][j], dp[i][k] + dp[k][j] + b[i] * b[k] * b[j]);\n    }\n\n  // The widest open interval contains every original balloon.\n  return dp[0][n - 1];\n}\n",
        },
        time: "O(n³)",
        space: "O(n²)",
    ...
    solutions: [
      {
        ...
        ts: "function minScoreTriangulation(values: number[]): number {\n  const n = values.length;\n  const memo = new Map<number, number>();\n  const go = (i: number, j: number): number => {\n    if (j - i < 2) return 0;\n    const key = i * n + j;\n    if (memo.has(key)) return memo.get(key);\n    let best = Infinity;\n    for (let k = i + 1; k < j; k++)\n      best = Math.min(best, go(i, k) + go(k, j) + values[i] * values[k] * values[j]);\n    memo.set(key, best);\n    return best;\n  };\n  return go(0, n - 1);\n}\n",
        commentedCode: {
          js: "function minScoreTriangulation(values) {\n  const n = values.length;\n  // Cache the minimum triangulation score inside each boundary edge (i, j).\n  const memo = new Map();\n\n  const go = (i, j) => {\n    // Fewer than three vertices cannot form a triangle.\n    if (j - i < 2) return 0;\n    const key = i * n + j;\n    if (memo.has(key)) return memo.get(key);\n\n    let best = Infinity;\n    // Choose k to form triangle (i, k, j), splitting the polygon into two ranges.\n    for (let k = i + 1; k < j; k++)\n      best = Math.min(best, go(i, k) + go(k, j) + values[i] * values[k] * values[j]);\n\n    memo.set(key, best);\n    return best;\n  };\n\n  // Use the polygon's first-to-last edge as the outer interval.\n  return go(0, n - 1);\n}\n",
          ts: "function minScoreTriangulation(values: number[]): number {\n  const n = values.length;\n  // Cache the minimum triangulation score inside each boundary edge (i, j).\n  const memo = new Map<number, number>();\n\n  const go = (i: number, j: number): number => {\n    // Fewer than three vertices cannot form a triangle.\n    if (j - i < 2) return 0;\n    const key = i * n + j;\n    if (memo.has(key)) return memo.get(key);\n\n    let best = Infinity;\n    // Choose k to form triangle (i, k, j), splitting the polygon into two ranges.\n    for (let k = i + 1; k < j; k++)\n      best = Math.min(best, go(i, k) + go(k, j) + values[i] * values[k] * values[j]);\n\n    memo.set(key, best);\n    return best;\n  };\n\n  // Use the polygon's first-to-last edge as the outer interval.\n  return go(0, n - 1);\n}\n",
        },
        time: "O(n³)",
        space: "O(n²)",
      },
      {
        ...
        ts: "function minScoreTriangulation(values: number[]): number {\n  const n = values.length;\n  const dp = Array.from({ length: n }, () => new Array(n).fill(0));\n  for (let len = 2; len < n; len++)\n    for (let i = 0; i + len < n; i++) {\n      const j = i + len;\n      dp[i][j] = Infinity;\n      for (let k = i + 1; k < j; k++)\n        dp[i][j] = Math.min(dp[i][j], dp[i][k] + dp[k][j] + values[i] * values[k] * values[j]);\n    }\n  return dp[0][n - 1];\n}\n",
        commentedCode: {
          js: "function minScoreTriangulation(values) {\n  const n = values.length;\n  // dp[i][j] is the minimum score for the polygonal range bounded by i and j.\n  const dp = Array.from({ length: n }, () => new Array(n).fill(0));\n\n  // Width two is the first range containing a triangle; then widen outward.\n  for (let len = 2; len < n; len++)\n    for (let i = 0; i + len < n; i++) {\n      const j = i + len;\n      // Initialize a nontrivial range before minimizing over valid apexes.\n      dp[i][j] = Infinity;\n      // Vertex k creates triangle (i, k, j) and two already-solved subranges.\n      for (let k = i + 1; k < j; k++)\n        dp[i][j] = Math.min(dp[i][j], dp[i][k] + dp[k][j] + values[i] * values[k] * values[j]);\n    }\n\n  // The widest interval represents the entire polygon.\n  return dp[0][n - 1];\n}\n",
          ts: "function minScoreTriangulation(values: number[]): number {\n  const n = values.length;\n  // dp[i][j] is the minimum score for the polygonal range bounded by i and j.\n  const dp = Array.from({ length: n }, () => new Array(n).fill(0));\n\n  // Width two is the first range containing a triangle; then widen outward.\n  for (let len = 2; len < n; len++)\n    for (let i = 0; i + len < n; i++) {\n      const j = i + len;\n      // Initialize a nontrivial range before minimizing over valid apexes.\n      dp[i][j] = Infinity;\n      // Vertex k creates triangle (i, k, j) and two already-solved subranges.\n      for (let k = i + 1; k < j; k++)\n        dp[i][j] = Math.min(dp[i][j], dp[i][k] + dp[k][j] + values[i] * values[k] * values[j]);\n    }\n\n  // The widest interval represents the entire polygon.\n  return dp[0][n - 1];\n}\n",
        },
        time: "O(n³)",
        space: "O(n²)",
