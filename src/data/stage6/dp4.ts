    ...
    solutions: [
      {
        ...
        commentedCode: {
          js: "function longestCommonSubsequence(a, b) {\n  // Cache answers for pairs of prefix lengths so each subproblem is solved once.\n  const memo = new Map();\n\n  // Return the LCS length between a[0..i) and b[0..j).\n  const go = (i, j) => {\n    // If either prefix is empty, they cannot share a character.\n    if (i === 0 || j === 0) return 0;\n\n    // Encode both prefix lengths in one collision-free numeric key.\n    const key = i * (b.length + 1) + j;\n    // Reuse a result reached through another sequence of choices.\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // Matching final characters extend the best subsequence of both shorter prefixes.\n    if (a[i - 1] === b[j - 1]) v = 1 + go(i - 1, j - 1);\n    // Otherwise, try discarding either final character and keep the better result.\n    else v = Math.max(go(i - 1, j), go(i, j - 1));\n\n    // Store the completed state before returning it.\n    memo.set(key, v);\n    return v;\n  };\n\n  // Solve the state containing both complete strings.\n  return go(a.length, b.length);\n}\n",
          ts: "function longestCommonSubsequence(a: string, b: string): number {\n  // Cache answers for pairs of prefix lengths so each subproblem is solved once.\n  const memo = new Map<number, number>();\n\n  // Return the LCS length between a[0..i) and b[0..j).\n  const go = (i: number, j: number): number => {\n    // If either prefix is empty, they cannot share a character.\n    if (i === 0 || j === 0) return 0;\n\n    // Encode both prefix lengths in one collision-free numeric key.\n    const key = i * (b.length + 1) + j;\n    // Reuse a result reached through another sequence of choices.\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // Matching final characters extend the best subsequence of both shorter prefixes.\n    if (a[i - 1] === b[j - 1]) v = 1 + go(i - 1, j - 1);\n    // Otherwise, try discarding either final character and keep the better result.\n    else v = Math.max(go(i - 1, j), go(i, j - 1));\n\n    // Store the completed state before returning it.\n    memo.set(key, v);\n    return v;\n  };\n\n  // Solve the state containing both complete strings.\n  return go(a.length, b.length);\n}\n",
        },
        time: "O(m·n)",
        space: "O(m·n)",
      },
      {
        ...
        ts: "function longestCommonSubsequence(a: string, b: string): number {\n  const m = a.length, n = b.length;\n  let prev = new Array(n + 1).fill(0);\n  for (let i = 1; i <= m; i++) {\n    const cur = new Array(n + 1).fill(0);\n    for (let j = 1; j <= n; j++) {\n      if (a[i - 1] === b[j - 1]) cur[j] = prev[j - 1] + 1;\n      else cur[j] = Math.max(prev[j], cur[j - 1]);\n    }\n    prev = cur;\n  }\n  return prev[n];\n}\n",
        commentedCode: {
          js: "function longestCommonSubsequence(a, b) {\n  // m and n are the two prefix dimensions of the conceptual DP table.\n  const m = a.length, n = b.length;\n  // The empty-a row contains zero LCS length for every b prefix.\n  let prev = new Array(n + 1).fill(0);\n\n  // Build the table one prefix of a at a time.\n  for (let i = 1; i <= m; i++) {\n    // cur[j] will describe a[0..i) against b[0..j).\n    const cur = new Array(n + 1).fill(0);\n    for (let j = 1; j <= n; j++) {\n      // A character match extends the diagonal state by one.\n      if (a[i - 1] === b[j - 1]) cur[j] = prev[j - 1] + 1;\n      // A mismatch keeps the better result after dropping one current character.\n      else cur[j] = Math.max(prev[j], cur[j - 1]);\n    }\n    // Only the completed row is needed to construct the next row.\n    prev = cur;\n  }\n\n  // The final cell compares both complete strings.\n  return prev[n];\n}\n",
          ts: "function longestCommonSubsequence(a: string, b: string): number {\n  // m and n are the two prefix dimensions of the conceptual DP table.\n  const m = a.length, n = b.length;\n  // The empty-a row contains zero LCS length for every b prefix.\n  let prev = new Array(n + 1).fill(0);\n\n  // Build the table one prefix of a at a time.\n  for (let i = 1; i <= m; i++) {\n    // cur[j] will describe a[0..i) against b[0..j).\n    const cur = new Array(n + 1).fill(0);\n    for (let j = 1; j <= n; j++) {\n      // A character match extends the diagonal state by one.\n      if (a[i - 1] === b[j - 1]) cur[j] = prev[j - 1] + 1;\n      // A mismatch keeps the better result after dropping one current character.\n      else cur[j] = Math.max(prev[j], cur[j - 1]);\n    }\n    // Only the completed row is needed to construct the next row.\n    prev = cur;\n  }\n\n  // The final cell compares both complete strings.\n  return prev[n];\n}\n",
        },
        time: "O(m·n)",
        space: "O(n)",
    ...
    solutions: [
      {
        ...
        ts: "function longestCommonSubstring(a: string, b: string): number {\n  const m = a.length, n = b.length;\n  let prev = new Array(n + 1).fill(0);\n  let best = 0;\n  for (let i = 1; i <= m; i++) {\n    const cur = new Array(n + 1).fill(0);\n    for (let j = 1; j <= n; j++) {\n      if (a[i - 1] === b[j - 1]) { cur[j] = prev[j - 1] + 1; best = Math.max(best, cur[j]); }\n    }\n    prev = cur;\n  }\n  return best;\n}\n",
        commentedCode: {
          js: "function longestCommonSubstring(a, b) {\n  // Use the two string lengths as the prefix-grid dimensions.\n  const m = a.length, n = b.length;\n  // prev[j] is the matching suffix length for the preceding a character.\n  let prev = new Array(n + 1).fill(0);\n  // The answer may occur anywhere in the table, so track its maximum.\n  let best = 0;\n\n  for (let i = 1; i <= m; i++) {\n    // Starting with zero also resets every mismatch, which breaks contiguity.\n    const cur = new Array(n + 1).fill(0);\n    for (let j = 1; j <= n; j++) {\n      // A match extends the contiguous run ending at both previous characters.\n      if (a[i - 1] === b[j - 1]) { cur[j] = prev[j - 1] + 1; best = Math.max(best, cur[j]); }\n    }\n    // Advance to the next character of a using only the current diagonal history.\n    prev = cur;\n  }\n\n  // Return the longest run found across all pairs of ending positions.\n  return best;\n}\n",
          ts: "function longestCommonSubstring(a: string, b: string): number {\n  // Use the two string lengths as the prefix-grid dimensions.\n  const m = a.length, n = b.length;\n  // prev[j] is the matching suffix length for the preceding a character.\n  let prev = new Array(n + 1).fill(0);\n  // The answer may occur anywhere in the table, so track its maximum.\n  let best = 0;\n\n  for (let i = 1; i <= m; i++) {\n    // Starting with zero also resets every mismatch, which breaks contiguity.\n    const cur = new Array(n + 1).fill(0);\n    for (let j = 1; j <= n; j++) {\n      // A match extends the contiguous run ending at both previous characters.\n      if (a[i - 1] === b[j - 1]) { cur[j] = prev[j - 1] + 1; best = Math.max(best, cur[j]); }\n    }\n    // Advance to the next character of a using only the current diagonal history.\n    prev = cur;\n  }\n\n  // Return the longest run found across all pairs of ending positions.\n  return best;\n}\n",
        },
        time: "O(m·n)",
        space: "O(n)",
      },
      {
        ...
        commentedCode: {
          js: "function longestCommonSubstring(a, b) {\n  // Cache matching-run lengths by their pair of ending prefix lengths.\n  const memo = new Map();\n\n  // Return the shared contiguous run ending at a[i - 1] and b[j - 1].\n  const run = (i, j) => {\n    // No run can extend beyond the start of either string.\n    if (i === 0 || j === 0) return 0;\n    // A mismatch immediately breaks a contiguous substring.\n    if (a[i - 1] !== b[j - 1]) return 0;\n\n    // Encode the two ending positions in one cache key.\n    const key = i * (b.length + 1) + j;\n    if (memo.has(key)) return memo.get(key);\n\n    // Matching ends add one to the diagonally preceding run.\n    const v = 1 + run(i - 1, j - 1);\n    memo.set(key, v);\n    return v;\n  };\n\n  // The longest substring may end at any pair of positions.\n  let best = 0;\n  for (let i = 1; i <= a.length; i++)\n    for (let j = 1; j <= b.length; j++) best = Math.max(best, run(i, j));\n\n  return best;\n}\n",
          ts: "function longestCommonSubstring(a: string, b: string): number {\n  // Cache matching-run lengths by their pair of ending prefix lengths.\n  const memo = new Map<number, number>();\n\n  // Return the shared contiguous run ending at a[i - 1] and b[j - 1].\n  const run = (i: number, j: number): number => {\n    // No run can extend beyond the start of either string.\n    if (i === 0 || j === 0) return 0;\n    // A mismatch immediately breaks a contiguous substring.\n    if (a[i - 1] !== b[j - 1]) return 0;\n\n    // Encode the two ending positions in one cache key.\n    const key = i * (b.length + 1) + j;\n    if (memo.has(key)) return memo.get(key);\n\n    // Matching ends add one to the diagonally preceding run.\n    const v = 1 + run(i - 1, j - 1);\n    memo.set(key, v);\n    return v;\n  };\n\n  // The longest substring may end at any pair of positions.\n  let best = 0;\n  for (let i = 1; i <= a.length; i++)\n    for (let j = 1; j <= b.length; j++) best = Math.max(best, run(i, j));\n\n  return best;\n}\n",
        },
        time: "O(m·n)",
        space: "O(m·n)",
    ...
    solutions: [
      {
        ...
        ts: "function editDistance(a: string, b: string): number {\n  const memo = new Map<number, number>();\n  const go = (i: number, j: number): number => {\n    if (i === 0) return j;\n    if (j === 0) return i;\n    const key = i * (b.length + 1) + j;\n    if (memo.has(key)) return memo.get(key);\n    let v;\n    if (a[i - 1] === b[j - 1]) v = go(i - 1, j - 1);\n    else v = 1 + Math.min(go(i - 1, j), go(i, j - 1), go(i - 1, j - 1));\n    memo.set(key, v);\n    return v;\n  };\n  return go(a.length, b.length);\n}\n",
        commentedCode: {
          js: "function editDistance(a, b) {\n  // Cache the distance for every pair of source and target prefix lengths.\n  const memo = new Map();\n\n  // Find the minimum edits needed to transform a[0..i) into b[0..j).\n  const go = (i, j) => {\n    // Converting an empty source requires inserting all j target characters.\n    if (i === 0) return j;\n    // Converting to an empty target requires deleting all i source characters.\n    if (j === 0) return i;\n\n    const key = i * (b.length + 1) + j;\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // Equal final characters need no edit, so move diagonally for free.\n    if (a[i - 1] === b[j - 1]) v = go(i - 1, j - 1);\n    // Otherwise pay once, then choose delete, insert, or replace respectively.\n    else v = 1 + Math.min(go(i - 1, j), go(i, j - 1), go(i - 1, j - 1));\n\n    memo.set(key, v);\n    return v;\n  };\n\n  return go(a.length, b.length);\n}\n",
          ts: "function editDistance(a: string, b: string): number {\n  // Cache the distance for every pair of source and target prefix lengths.\n  const memo = new Map<number, number>();\n\n  // Find the minimum edits needed to transform a[0..i) into b[0..j).\n  const go = (i: number, j: number): number => {\n    // Converting an empty source requires inserting all j target characters.\n    if (i === 0) return j;\n    // Converting to an empty target requires deleting all i source characters.\n    if (j === 0) return i;\n\n    const key = i * (b.length + 1) + j;\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // Equal final characters need no edit, so move diagonally for free.\n    if (a[i - 1] === b[j - 1]) v = go(i - 1, j - 1);\n    // Otherwise pay once, then choose delete, insert, or replace respectively.\n    else v = 1 + Math.min(go(i - 1, j), go(i, j - 1), go(i - 1, j - 1));\n\n    memo.set(key, v);\n    return v;\n  };\n\n  return go(a.length, b.length);\n}\n",
        },
        time: "O(m·n)",
        space: "O(m·n)",
      },
      {
        ...
        ts: "function editDistance(a: string, b: string): number {\n  const m = a.length, n = b.length;\n  let prev = Array.from({ length: n + 1 }, (_, j) => j);\n  for (let i = 1; i <= m; i++) {\n    const cur = new Array(n + 1);\n    cur[0] = i;\n    for (let j = 1; j <= n; j++) {\n      if (a[i - 1] === b[j - 1]) cur[j] = prev[j - 1];\n      else cur[j] = 1 + Math.min(prev[j], cur[j - 1], prev[j - 1]);\n    }\n    prev = cur;\n  }\n  return prev[n];\n}\n",
        commentedCode: {
          js: "function editDistance(a, b) {\n  // m and n define all pairs of source and target prefixes.\n  const m = a.length, n = b.length;\n  // Turning empty a into each b prefix takes j insertions.\n  let prev = Array.from({ length: n + 1 }, (_, j) => j);\n\n  for (let i = 1; i <= m; i++) {\n    const cur = new Array(n + 1);\n    // Turning an i-character source into empty b takes i deletions.\n    cur[0] = i;\n    for (let j = 1; j <= n; j++) {\n      // Equal final characters inherit the diagonal distance without another edit.\n      if (a[i - 1] === b[j - 1]) cur[j] = prev[j - 1];\n      // Otherwise add one to the cheapest delete, insert, or replace predecessor.\n      else cur[j] = 1 + Math.min(prev[j], cur[j - 1], prev[j - 1]);\n    }\n    // Roll the completed prefix row forward.\n    prev = cur;\n  }\n\n  return prev[n];\n}\n",
          ts: "function editDistance(a: string, b: string): number {\n  // m and n define all pairs of source and target prefixes.\n  const m = a.length, n = b.length;\n  // Turning empty a into each b prefix takes j insertions.\n  let prev = Array.from({ length: n + 1 }, (_, j) => j);\n\n  for (let i = 1; i <= m; i++) {\n    const cur = new Array(n + 1);\n    // Turning an i-character source into empty b takes i deletions.\n    cur[0] = i;\n    for (let j = 1; j <= n; j++) {\n      // Equal final characters inherit the diagonal distance without another edit.\n      if (a[i - 1] === b[j - 1]) cur[j] = prev[j - 1];\n      // Otherwise add one to the cheapest delete, insert, or replace predecessor.\n      else cur[j] = 1 + Math.min(prev[j], cur[j - 1], prev[j - 1]);\n    }\n    // Roll the completed prefix row forward.\n    prev = cur;\n  }\n\n  return prev[n];\n}\n",
        },
        time: "O(m·n)",
        space: "O(n)",
    ...
    solutions: [
      {
        ...
        ts: "function longestPalindromeSubseq(s: string): number {\n  const n = s.length;\n  const memo = new Map<number, number>();\n  const go = (i: number, j: number): number => {\n    if (i > j) return 0;\n    if (i === j) return 1;\n    const key = i * n + j;\n    if (memo.has(key)) return memo.get(key);\n    let v;\n    if (s[i] === s[j]) v = 2 + go(i + 1, j - 1);\n    else v = Math.max(go(i + 1, j), go(i, j - 1));\n    memo.set(key, v);\n    return v;\n  };\n  return go(0, n - 1);\n}\n",
        commentedCode: {
          js: "function longestPalindromeSubseq(s) {\n  const n = s.length;\n  // Cache the best palindromic subsequence length for each interval.\n  const memo = new Map();\n\n  // Solve the inclusive interval s[i..j] by moving its two boundaries inward.\n  const go = (i, j) => {\n    // Crossing the boundaries leaves an empty subsequence.\n    if (i > j) return 0;\n    // A single character is itself a palindrome of length one.\n    if (i === j) return 1;\n\n    const key = i * n + j;\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // Matching ends can both surround the best subsequence inside them.\n    if (s[i] === s[j]) v = 2 + go(i + 1, j - 1);\n    // Otherwise exclude either end and preserve the better remaining interval.\n    else v = Math.max(go(i + 1, j), go(i, j - 1));\n\n    memo.set(key, v);\n    return v;\n  };\n\n  return go(0, n - 1);\n}\n",
          ts: "function longestPalindromeSubseq(s: string): number {\n  const n = s.length;\n  // Cache the best palindromic subsequence length for each interval.\n  const memo = new Map<number, number>();\n\n  // Solve the inclusive interval s[i..j] by moving its two boundaries inward.\n  const go = (i: number, j: number): number => {\n    // Crossing the boundaries leaves an empty subsequence.\n    if (i > j) return 0;\n    // A single character is itself a palindrome of length one.\n    if (i === j) return 1;\n\n    const key = i * n + j;\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // Matching ends can both surround the best subsequence inside them.\n    if (s[i] === s[j]) v = 2 + go(i + 1, j - 1);\n    // Otherwise exclude either end and preserve the better remaining interval.\n    else v = Math.max(go(i + 1, j), go(i, j - 1));\n\n    memo.set(key, v);\n    return v;\n  };\n\n  return go(0, n - 1);\n}\n",
        },
        time: "O(n²)",
        space: "O(n²)",
      },
      {
        ...
        ts: "function longestPalindromeSubseq(s: string): number {\n  const t = s.split('').reverse().join('');\n  const n = s.length;\n  let prev = new Array(n + 1).fill(0);\n  for (let i = 1; i <= n; i++) {\n    const cur = new Array(n + 1).fill(0);\n    for (let j = 1; j <= n; j++) {\n      if (s[i - 1] === t[j - 1]) cur[j] = prev[j - 1] + 1;\n      else cur[j] = Math.max(prev[j], cur[j - 1]);\n    }\n    prev = cur;\n  }\n  return prev[n];\n}\n",
        commentedCode: {
          js: "function longestPalindromeSubseq(s) {\n  // A palindrome appears unchanged in the reversed string.\n  const t = s.split('').reverse().join('');\n  const n = s.length;\n  // Compute LCS(s, t) with only the preceding row of the prefix table.\n  let prev = new Array(n + 1).fill(0);\n\n  for (let i = 1; i <= n; i++) {\n    const cur = new Array(n + 1).fill(0);\n    for (let j = 1; j <= n; j++) {\n      // Matching characters extend the common subsequence on the diagonal.\n      if (s[i - 1] === t[j - 1]) cur[j] = prev[j - 1] + 1;\n      // Otherwise skip one character from either prefix.\n      else cur[j] = Math.max(prev[j], cur[j - 1]);\n    }\n    prev = cur;\n  }\n\n  // This LCS length equals the longest palindromic subsequence length.\n  return prev[n];\n}\n",
          ts: "function longestPalindromeSubseq(s: string): number {\n  // A palindrome appears unchanged in the reversed string.\n  const t = s.split('').reverse().join('');\n  const n = s.length;\n  // Compute LCS(s, t) with only the preceding row of the prefix table.\n  let prev = new Array(n + 1).fill(0);\n\n  for (let i = 1; i <= n; i++) {\n    const cur = new Array(n + 1).fill(0);\n    for (let j = 1; j <= n; j++) {\n      // Matching characters extend the common subsequence on the diagonal.\n      if (s[i - 1] === t[j - 1]) cur[j] = prev[j - 1] + 1;\n      // Otherwise skip one character from either prefix.\n      else cur[j] = Math.max(prev[j], cur[j - 1]);\n    }\n    prev = cur;\n  }\n\n  // This LCS length equals the longest palindromic subsequence length.\n  return prev[n];\n}\n",
        },
        time: "O(n²)",
        space: "O(n)",
    ...
    solutions: [
      {
        ...
        commentedCode: {
          js: "function minInsertionsPalindrome(s) {\n  const n = s.length;\n  // Cache the longest palindromic subsequence for each inclusive interval.\n  const memo = new Map();\n\n  const lps = (i, j) => {\n    // An empty interval contributes no kept characters.\n    if (i > j) return 0;\n    // A single character already forms a palindrome.\n    if (i === j) return 1;\n\n    const key = i * n + j;\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // Matching ends can both remain around the best inner palindrome.\n    if (s[i] === s[j]) v = 2 + lps(i + 1, j - 1);\n    // Otherwise one end must be excluded from the kept subsequence.\n    else v = Math.max(lps(i + 1, j), lps(i, j - 1));\n\n    memo.set(key, v);\n    return v;\n  };\n\n  // Every character outside the LPS needs a matching character inserted.\n  return n - lps(0, n - 1);\n}\n",
          ts: "function minInsertionsPalindrome(s: string): number {\n  const n = s.length;\n  // Cache the longest palindromic subsequence for each inclusive interval.\n  const memo = new Map<number, number>();\n\n  const lps = (i: number, j: number): number => {\n    // An empty interval contributes no kept characters.\n    if (i > j) return 0;\n    // A single character already forms a palindrome.\n    if (i === j) return 1;\n\n    const key = i * n + j;\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // Matching ends can both remain around the best inner palindrome.\n    if (s[i] === s[j]) v = 2 + lps(i + 1, j - 1);\n    // Otherwise one end must be excluded from the kept subsequence.\n    else v = Math.max(lps(i + 1, j), lps(i, j - 1));\n\n    memo.set(key, v);\n    return v;\n  };\n\n  // Every character outside the LPS needs a matching character inserted.\n  return n - lps(0, n - 1);\n}\n",
        },
        time: "O(n²)",
        space: "O(n²)",
      },
      {
        ...
        ts: "function minInsertionsPalindrome(s: string): number {\n  const n = s.length;\n  if (n === 0) return 0;\n  const dp = Array.from({ length: n }, () => new Array(n).fill(0));\n  for (let len = 2; len <= n; len++)\n    for (let i = 0; i + len - 1 < n; i++) {\n      const j = i + len - 1;\n      if (s[i] === s[j]) dp[i][j] = dp[i + 1][j - 1];\n      else dp[i][j] = 1 + Math.min(dp[i + 1][j], dp[i][j - 1]);\n    }\n  return dp[0][n - 1];\n}\n",
        commentedCode: {
          js: "function minInsertionsPalindrome(s) {\n  const n = s.length;\n  // The empty string needs no insertions and has no table cell to return.\n  if (n === 0) return 0;\n\n  // dp[i][j] is the minimum insertions needed for inclusive interval s[i..j].\n  // Length-zero and length-one intervals start at zero insertions.\n  const dp = Array.from({ length: n }, () => new Array(n).fill(0));\n\n  // Increasing length ensures each strictly smaller interval is already known.\n  for (let len = 2; len <= n; len++)\n    for (let i = 0; i + len - 1 < n; i++) {\n      const j = i + len - 1;\n      // Matching ends need only whatever insertions the interior needs.\n      if (s[i] === s[j]) dp[i][j] = dp[i + 1][j - 1];\n      // Otherwise insert a mirror for one end and solve the cheaper remainder.\n      else dp[i][j] = 1 + Math.min(dp[i + 1][j], dp[i][j - 1]);\n    }\n\n  return dp[0][n - 1];\n}\n",
          ts: "function minInsertionsPalindrome(s: string): number {\n  const n = s.length;\n  // The empty string needs no insertions and has no table cell to return.\n  if (n === 0) return 0;\n\n  // dp[i][j] is the minimum insertions needed for inclusive interval s[i..j].\n  // Length-zero and length-one intervals start at zero insertions.\n  const dp = Array.from({ length: n }, () => new Array(n).fill(0));\n\n  // Increasing length ensures each strictly smaller interval is already known.\n  for (let len = 2; len <= n; len++)\n    for (let i = 0; i + len - 1 < n; i++) {\n      const j = i + len - 1;\n      // Matching ends need only whatever insertions the interior needs.\n      if (s[i] === s[j]) dp[i][j] = dp[i + 1][j - 1];\n      // Otherwise insert a mirror for one end and solve the cheaper remainder.\n      else dp[i][j] = 1 + Math.min(dp[i + 1][j], dp[i][j - 1]);\n    }\n\n  return dp[0][n - 1];\n}\n",
        },
        time: "O(n²)",
        space: "O(n²)",
    ...
    solutions: [
      {
        ...
        commentedCode: {
          js: "function longestPalindromicSubstringLength(s) {\n  // Handle the only case with no center to inspect.\n  if (s.length === 0) return 0;\n  // Every non-empty string contains a one-character palindrome.\n  let best = 1;\n\n  // Expand symmetrically and return the matched span around this center.\n  const expand = (l, r) => {\n    // Continue only while both pointers are valid and their characters match.\n    while (l >= 0 && r < s.length && s[l] === s[r]) { l--; r++; }\n    // The pointers stopped one position outside the palindrome on each side.\n    return r - l - 1;\n  };\n\n  for (let i = 0; i < s.length; i++) {\n    // Check the odd center (i, i) and the even center (i, i + 1).\n    best = Math.max(best, expand(i, i), expand(i, i + 1));\n  }\n\n  return best;\n}\n",
          ts: "function longestPalindromicSubstringLength(s: string): number {\n  // Handle the only case with no center to inspect.\n  if (s.length === 0) return 0;\n  // Every non-empty string contains a one-character palindrome.\n  let best = 1;\n\n  // Expand symmetrically and return the matched span around this center.\n  const expand = (l: number, r: number): number => {\n    // Continue only while both pointers are valid and their characters match.\n    while (l >= 0 && r < s.length && s[l] === s[r]) { l--; r++; }\n    // The pointers stopped one position outside the palindrome on each side.\n    return r - l - 1;\n  };\n\n  for (let i = 0; i < s.length; i++) {\n    // Check the odd center (i, i) and the even center (i, i + 1).\n    best = Math.max(best, expand(i, i), expand(i, i + 1));\n  }\n\n  return best;\n}\n",
        },
        time: "O(n²)",
        space: "O(1)",
      },
      {
        ...
        ts: "function longestPalindromicSubstringLength(s: string): number {\n  const n = s.length;\n  if (n === 0) return 0;\n  const dp = Array.from({ length: n }, () => new Array(n).fill(false));\n  let best = 1;\n  for (let i = 0; i < n; i++) dp[i][i] = true;\n  for (let len = 2; len <= n; len++)\n    for (let i = 0; i + len - 1 < n; i++) {\n      const j = i + len - 1;\n      if (s[i] === s[j] && (len < 3 || dp[i + 1][j - 1])) {\n        dp[i][j] = true;\n        best = Math.max(best, len);\n      }\n    }\n  return best;\n}\n",
        commentedCode: {
          js: "function longestPalindromicSubstringLength(s) {\n  const n = s.length;\n  if (n === 0) return 0;\n\n  // dp[i][j] records whether the inclusive substring s[i..j] is a palindrome.\n  const dp = Array.from({ length: n }, () => new Array(n).fill(false));\n  let best = 1;\n\n  // Every one-character interval is palindromic.\n  for (let i = 0; i < n; i++) dp[i][i] = true;\n  // Build longer intervals only after their interiors have been evaluated.\n  for (let len = 2; len <= n; len++)\n    for (let i = 0; i + len - 1 < n; i++) {\n      const j = i + len - 1;\n      // Equal ends form a palindrome when the interval is tiny or its interior is one.\n      if (s[i] === s[j] && (len < 3 || dp[i + 1][j - 1])) {\n        dp[i][j] = true;\n        best = Math.max(best, len);\n      }\n    }\n\n  return best;\n}\n",
          ts: "function longestPalindromicSubstringLength(s: string): number {\n  const n = s.length;\n  if (n === 0) return 0;\n\n  // dp[i][j] records whether the inclusive substring s[i..j] is a palindrome.\n  const dp = Array.from({ length: n }, () => new Array(n).fill(false));\n  let best = 1;\n\n  // Every one-character interval is palindromic.\n  for (let i = 0; i < n; i++) dp[i][i] = true;\n  // Build longer intervals only after their interiors have been evaluated.\n  for (let len = 2; len <= n; len++)\n    for (let i = 0; i + len - 1 < n; i++) {\n      const j = i + len - 1;\n      // Equal ends form a palindrome when the interval is tiny or its interior is one.\n      if (s[i] === s[j] && (len < 3 || dp[i + 1][j - 1])) {\n        dp[i][j] = true;\n        best = Math.max(best, len);\n      }\n    }\n\n  return best;\n}\n",
        },
        time: "O(n²)",
        space: "O(n²)",
    ...
    solutions: [
      {
        ...
        ts: "function deleteDistance(a: string, b: string): number {\n  const m = a.length, n = b.length;\n  let prev = new Array(n + 1).fill(0);\n  for (let i = 1; i <= m; i++) {\n    const cur = new Array(n + 1).fill(0);\n    for (let j = 1; j <= n; j++) {\n      if (a[i - 1] === b[j - 1]) cur[j] = prev[j - 1] + 1;\n      else cur[j] = Math.max(prev[j], cur[j - 1]);\n    }\n    prev = cur;\n  }\n  return m + n - 2 * prev[n];\n}\n",
        commentedCode: {
          js: "function deleteDistance(a, b) {\n  const m = a.length, n = b.length;\n  // Find the LCS length with a rolling prefix-DP row.\n  let prev = new Array(n + 1).fill(0);\n\n  for (let i = 1; i <= m; i++) {\n    const cur = new Array(n + 1).fill(0);\n    for (let j = 1; j <= n; j++) {\n      // A shared character can remain in both strings.\n      if (a[i - 1] === b[j - 1]) cur[j] = prev[j - 1] + 1;\n      // Otherwise skip one current character and keep the longer common subsequence.\n      else cur[j] = Math.max(prev[j], cur[j - 1]);\n    }\n    prev = cur;\n  }\n\n  // Delete every non-LCS character: once from a and once from b.\n  return m + n - 2 * prev[n];\n}\n",
          ts: "function deleteDistance(a: string, b: string): number {\n  const m = a.length, n = b.length;\n  // Find the LCS length with a rolling prefix-DP row.\n  let prev = new Array(n + 1).fill(0);\n\n  for (let i = 1; i <= m; i++) {\n    const cur = new Array(n + 1).fill(0);\n    for (let j = 1; j <= n; j++) {\n      // A shared character can remain in both strings.\n      if (a[i - 1] === b[j - 1]) cur[j] = prev[j - 1] + 1;\n      // Otherwise skip one current character and keep the longer common subsequence.\n      else cur[j] = Math.max(prev[j], cur[j - 1]);\n    }\n    prev = cur;\n  }\n\n  // Delete every non-LCS character: once from a and once from b.\n  return m + n - 2 * prev[n];\n}\n",
        },
        time: "O(m·n)",
        space: "O(n)",
      },
      {
        ...
        ts: "function deleteDistance(a: string, b: string): number {\n  const memo = new Map<number, number>();\n  const go = (i: number, j: number): number => {\n    if (i === 0) return j;\n    if (j === 0) return i;\n    const key = i * (b.length + 1) + j;\n    if (memo.has(key)) return memo.get(key);\n    let v;\n    if (a[i - 1] === b[j - 1]) v = go(i - 1, j - 1);\n    else v = 1 + Math.min(go(i - 1, j), go(i, j - 1));\n    memo.set(key, v);\n    return v;\n  };\n  return go(a.length, b.length);\n}\n",
        commentedCode: {
          js: "function deleteDistance(a, b) {\n  // Cache the minimum deletions for each pair of prefix lengths.\n  const memo = new Map();\n\n  const go = (i, j) => {\n    // If a is empty, every remaining character of b must be deleted.\n    if (i === 0) return j;\n    // If b is empty, every remaining character of a must be deleted.\n    if (j === 0) return i;\n\n    const key = i * (b.length + 1) + j;\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // Matching final characters can stay, so shrink both prefixes for free.\n    if (a[i - 1] === b[j - 1]) v = go(i - 1, j - 1);\n    // Otherwise pay for deleting from a or b and choose the cheaper result.\n    else v = 1 + Math.min(go(i - 1, j), go(i, j - 1));\n\n    memo.set(key, v);\n    return v;\n  };\n\n  return go(a.length, b.length);\n}\n",
          ts: "function deleteDistance(a: string, b: string): number {\n  // Cache the minimum deletions for each pair of prefix lengths.\n  const memo = new Map<number, number>();\n\n  const go = (i: number, j: number): number => {\n    // If a is empty, every remaining character of b must be deleted.\n    if (i === 0) return j;\n    // If b is empty, every remaining character of a must be deleted.\n    if (j === 0) return i;\n\n    const key = i * (b.length + 1) + j;\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // Matching final characters can stay, so shrink both prefixes for free.\n    if (a[i - 1] === b[j - 1]) v = go(i - 1, j - 1);\n    // Otherwise pay for deleting from a or b and choose the cheaper result.\n    else v = 1 + Math.min(go(i - 1, j), go(i, j - 1));\n\n    memo.set(key, v);\n    return v;\n  };\n\n  return go(a.length, b.length);\n}\n",
        },
        time: "O(m·n)",
        space: "O(m·n)",
    ...
    solutions: [
      {
        ...
        commentedCode: {
          js: "function countPalindromicSubstrings(s) {\n  // Count every valid start/end interval, including duplicates in content.\n  let count = 0;\n\n  // Count all palindromes sharing one odd or even center.\n  const expand = (l, r) => {\n    // Each successful expansion discovers exactly one new palindromic interval.\n    while (l >= 0 && r < s.length && s[l] === s[r]) { count++; l--; r++; }\n  };\n\n  // Every palindrome has either a character center or a gap center.\n  for (let i = 0; i < s.length; i++) { expand(i, i); expand(i, i + 1); }\n\n  return count;\n}\n",
          ts: "function countPalindromicSubstrings(s: string): number {\n  // Count every valid start/end interval, including duplicates in content.\n  let count = 0;\n\n  // Count all palindromes sharing one odd or even center.\n  const expand = (l: number, r: number): void => {\n    // Each successful expansion discovers exactly one new palindromic interval.\n    while (l >= 0 && r < s.length && s[l] === s[r]) { count++; l--; r++; }\n  };\n\n  // Every palindrome has either a character center or a gap center.\n  for (let i = 0; i < s.length; i++) { expand(i, i); expand(i, i + 1); }\n\n  return count;\n}\n",
        },
        time: "O(n²)",
        space: "O(1)",
      },
      {
        ...
        ts: "function countPalindromicSubstrings(s: string): number {\n  const n = s.length;\n  if (n === 0) return 0;\n  const dp = Array.from({ length: n }, () => new Array(n).fill(false));\n  let count = 0;\n  for (let i = 0; i < n; i++) { dp[i][i] = true; count++; }\n  for (let len = 2; len <= n; len++)\n    for (let i = 0; i + len - 1 < n; i++) {\n      const j = i + len - 1;\n      if (s[i] === s[j] && (len < 3 || dp[i + 1][j - 1])) { dp[i][j] = true; count++; }\n    }\n  return count;\n}\n",
        commentedCode: {
          js: "function countPalindromicSubstrings(s) {\n  const n = s.length;\n  if (n === 0) return 0;\n\n  // dp[i][j] says whether the inclusive interval s[i..j] is palindromic.\n  const dp = Array.from({ length: n }, () => new Array(n).fill(false));\n  let count = 0;\n\n  // Seed and count every single-character palindrome.\n  for (let i = 0; i < n; i++) { dp[i][i] = true; count++; }\n  // Grow intervals so each interior state is available before its enclosing range.\n  for (let len = 2; len <= n; len++)\n    for (let i = 0; i + len - 1 < n; i++) {\n      const j = i + len - 1;\n      // Equal ends are enough for length two; longer ranges also need a palindromic interior.\n      if (s[i] === s[j] && (len < 3 || dp[i + 1][j - 1])) { dp[i][j] = true; count++; }\n    }\n\n  return count;\n}\n",
          ts: "function countPalindromicSubstrings(s: string): number {\n  const n = s.length;\n  if (n === 0) return 0;\n\n  // dp[i][j] says whether the inclusive interval s[i..j] is palindromic.\n  const dp = Array.from({ length: n }, () => new Array(n).fill(false));\n  let count = 0;\n\n  // Seed and count every single-character palindrome.\n  for (let i = 0; i < n; i++) { dp[i][i] = true; count++; }\n  // Grow intervals so each interior state is available before its enclosing range.\n  for (let len = 2; len <= n; len++)\n    for (let i = 0; i + len - 1 < n; i++) {\n      const j = i + len - 1;\n      // Equal ends are enough for length two; longer ranges also need a palindromic interior.\n      if (s[i] === s[j] && (len < 3 || dp[i + 1][j - 1])) { dp[i][j] = true; count++; }\n    }\n\n  return count;\n}\n",
        },
        time: "O(n²)",
        space: "O(n²)",
    ...
    solutions: [
      {
        ...
        ts: "function numDistinctSubsequences(s: string, t: string): number {\n  const memo = new Map<number, number>();\n  const go = (i: number, j: number): number => {\n    if (j === t.length) return 1;\n    if (i === s.length) return 0;\n    const key = i * (t.length + 1) + j;\n    if (memo.has(key)) return memo.get(key);\n    let v = go(i + 1, j);\n    if (s[i] === t[j]) v += go(i + 1, j + 1);\n    memo.set(key, v);\n    return v;\n  };\n  return go(0, 0);\n}\n",
        commentedCode: {
          js: "function numDistinctSubsequences(s, t) {\n  // Cache the number of matches possible from each pair of current indices.\n  const memo = new Map();\n\n  // Count ways to form t[j..] using characters selected from s[i..].\n  const go = (i, j) => {\n    // Finishing t produces one valid subsequence: the choices made so far.\n    if (j === t.length) return 1;\n    // Exhausting s before t means this branch cannot complete a match.\n    if (i === s.length) return 0;\n\n    const key = i * (t.length + 1) + j;\n    if (memo.has(key)) return memo.get(key);\n\n    // Every state may skip the current source character.\n    let v = go(i + 1, j);\n    // A matching character also creates branches that consume both characters.\n    if (s[i] === t[j]) v += go(i + 1, j + 1);\n\n    memo.set(key, v);\n    return v;\n  };\n\n  return go(0, 0);\n}\n",
          ts: "function numDistinctSubsequences(s: string, t: string): number {\n  // Cache the number of matches possible from each pair of current indices.\n  const memo = new Map<number, number>();\n\n  // Count ways to form t[j..] using characters selected from s[i..].\n  const go = (i: number, j: number): number => {\n    // Finishing t produces one valid subsequence: the choices made so far.\n    if (j === t.length) return 1;\n    // Exhausting s before t means this branch cannot complete a match.\n    if (i === s.length) return 0;\n\n    const key = i * (t.length + 1) + j;\n    if (memo.has(key)) return memo.get(key);\n\n    // Every state may skip the current source character.\n    let v = go(i + 1, j);\n    // A matching character also creates branches that consume both characters.\n    if (s[i] === t[j]) v += go(i + 1, j + 1);\n\n    memo.set(key, v);\n    return v;\n  };\n\n  return go(0, 0);\n}\n",
        },
        time: "O(m·n)",
        space: "O(m·n)",
      },
      {
        ...
        ts: "function numDistinctSubsequences(s: string, t: string): number {\n  const m = s.length, n = t.length;\n  let prev = new Array(n + 1).fill(0);\n  prev[0] = 1;\n  for (let i = 1; i <= m; i++) {\n    const cur = new Array(n + 1).fill(0);\n    cur[0] = 1;\n    for (let j = 1; j <= n; j++) {\n      cur[j] = prev[j];\n      if (s[i - 1] === t[j - 1]) cur[j] += prev[j - 1];\n    }\n    prev = cur;\n  }\n  return prev[n];\n}\n",
        commentedCode: {
          js: "function numDistinctSubsequences(s, t) {\n  const m = s.length, n = t.length;\n  // prev[j] counts ways to build the first j target characters from a source prefix.\n  let prev = new Array(n + 1).fill(0);\n  // Every source prefix forms the empty target in exactly one way.\n  prev[0] = 1;\n\n  for (let i = 1; i <= m; i++) {\n    const cur = new Array(n + 1).fill(0);\n    // Skipping every available source character still forms empty t once.\n    cur[0] = 1;\n    for (let j = 1; j <= n; j++) {\n      // First count ways that skip s[i - 1].\n      cur[j] = prev[j];\n      // If characters match, also count ways that pair them and extend t's shorter prefix.\n      if (s[i - 1] === t[j - 1]) cur[j] += prev[j - 1];\n    }\n    prev = cur;\n  }\n\n  return prev[n];\n}\n",
          ts: "function numDistinctSubsequences(s: string, t: string): number {\n  const m = s.length, n = t.length;\n  // prev[j] counts ways to build the first j target characters from a source prefix.\n  let prev = new Array(n + 1).fill(0);\n  // Every source prefix forms the empty target in exactly one way.\n  prev[0] = 1;\n\n  for (let i = 1; i <= m; i++) {\n    const cur = new Array(n + 1).fill(0);\n    // Skipping every available source character still forms empty t once.\n    cur[0] = 1;\n    for (let j = 1; j <= n; j++) {\n      // First count ways that skip s[i - 1].\n      cur[j] = prev[j];\n      // If characters match, also count ways that pair them and extend t's shorter prefix.\n      if (s[i - 1] === t[j - 1]) cur[j] += prev[j - 1];\n    }\n    prev = cur;\n  }\n\n  return prev[n];\n}\n",
        },
        time: "O(m·n)",
        space: "O(n)",
    ...
    solutions: [
      {
        ...
        ts: "function shortestCommonSupersequenceLength(a: string, b: string): number {\n  const m = a.length, n = b.length;\n  let prev = new Array(n + 1).fill(0);\n  for (let i = 1; i <= m; i++) {\n    const cur = new Array(n + 1).fill(0);\n    for (let j = 1; j <= n; j++) {\n      if (a[i - 1] === b[j - 1]) cur[j] = prev[j - 1] + 1;\n      else cur[j] = Math.max(prev[j], cur[j - 1]);\n    }\n    prev = cur;\n  }\n  return m + n - prev[n];\n}\n",
        commentedCode: {
          js: "function shortestCommonSupersequenceLength(a, b) {\n  const m = a.length, n = b.length;\n  // Compute the LCS so shared characters can be counted only once.\n  let prev = new Array(n + 1).fill(0);\n\n  for (let i = 1; i <= m; i++) {\n    const cur = new Array(n + 1).fill(0);\n    for (let j = 1; j <= n; j++) {\n      // Matching characters extend the common subsequence.\n      if (a[i - 1] === b[j - 1]) cur[j] = prev[j - 1] + 1;\n      // Otherwise preserve the longer LCS after dropping one current character.\n      else cur[j] = Math.max(prev[j], cur[j - 1]);\n    }\n    prev = cur;\n  }\n\n  // Start with both lengths, then subtract the shared characters counted twice.\n  return m + n - prev[n];\n}\n",
          ts: "function shortestCommonSupersequenceLength(a: string, b: string): number {\n  const m = a.length, n = b.length;\n  // Compute the LCS so shared characters can be counted only once.\n  let prev = new Array(n + 1).fill(0);\n\n  for (let i = 1; i <= m; i++) {\n    const cur = new Array(n + 1).fill(0);\n    for (let j = 1; j <= n; j++) {\n      // Matching characters extend the common subsequence.\n      if (a[i - 1] === b[j - 1]) cur[j] = prev[j - 1] + 1;\n      // Otherwise preserve the longer LCS after dropping one current character.\n      else cur[j] = Math.max(prev[j], cur[j - 1]);\n    }\n    prev = cur;\n  }\n\n  // Start with both lengths, then subtract the shared characters counted twice.\n  return m + n - prev[n];\n}\n",
        },
        time: "O(m·n)",
        space: "O(n)",
      },
      {
        ...
        ts: "function shortestCommonSupersequenceLength(a: string, b: string): number {\n  const memo = new Map<number, number>();\n  const go = (i: number, j: number): number => {\n    if (i === 0) return j;\n    if (j === 0) return i;\n    const key = i * (b.length + 1) + j;\n    if (memo.has(key)) return memo.get(key);\n    let v;\n    if (a[i - 1] === b[j - 1]) v = 1 + go(i - 1, j - 1);\n    else v = 1 + Math.min(go(i - 1, j), go(i, j - 1));\n    memo.set(key, v);\n    return v;\n  };\n  return go(a.length, b.length);\n}\n",
        commentedCode: {
          js: "function shortestCommonSupersequenceLength(a, b) {\n  // Cache the shortest merged length for every pair of prefix lengths.\n  const memo = new Map();\n\n  const go = (i, j) => {\n    // If a is empty, append all j remaining characters of b.\n    if (i === 0) return j;\n    // If b is empty, append all i remaining characters of a.\n    if (j === 0) return i;\n\n    const key = i * (b.length + 1) + j;\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // Equal final characters are written once while both prefixes shrink.\n    if (a[i - 1] === b[j - 1]) v = 1 + go(i - 1, j - 1);\n    // Otherwise write one final character and choose which prefix to shrink.\n    else v = 1 + Math.min(go(i - 1, j), go(i, j - 1));\n\n    memo.set(key, v);\n    return v;\n  };\n\n  return go(a.length, b.length);\n}\n",
          ts: "function shortestCommonSupersequenceLength(a: string, b: string): number {\n  // Cache the shortest merged length for every pair of prefix lengths.\n  const memo = new Map<number, number>();\n\n  const go = (i: number, j: number): number => {\n    // If a is empty, append all j remaining characters of b.\n    if (i === 0) return j;\n    // If b is empty, append all i remaining characters of a.\n    if (j === 0) return i;\n\n    const key = i * (b.length + 1) + j;\n    if (memo.has(key)) return memo.get(key);\n\n    let v;\n    // Equal final characters are written once while both prefixes shrink.\n    if (a[i - 1] === b[j - 1]) v = 1 + go(i - 1, j - 1);\n    // Otherwise write one final character and choose which prefix to shrink.\n    else v = 1 + Math.min(go(i - 1, j), go(i, j - 1));\n\n    memo.set(key, v);\n    return v;\n  };\n\n  return go(a.length, b.length);\n}\n",
        },
        time: "O(m·n)",
        space: "O(m·n)",
