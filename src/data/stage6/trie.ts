    ...
    solutions: [
      {
        ...
        ts: "function wordExists(words: string[], query: string): boolean {\n  const root: any = {};\n  for (const w of words) {\n    let node = root;\n    for (const c of w) { node[c] = node[c] || {}; node = node[c]; }\n    node.end = true;\n  }\n  let node = root;\n  for (const c of query) { if (!node[c]) return false; node = node[c]; }\n  return node.end === true;\n}\n",
        commentedCode: {
          js: "function wordExists(words, query) {\n  // The root represents the empty prefix shared by every inserted word.\n  const root = {};\n\n  // Insert each dictionary word one character at a time.\n  for (const w of words) {\n    // Begin this word at the root of the trie.\n    let node = root;\n    // Reuse an existing character edge or create it, then descend through it.\n    for (const c of w) { node[c] = node[c] || {}; node = node[c]; }\n    // Distinguish a complete word from a prefix such as app inside apple.\n    node.end = true;\n  }\n\n  // Match the query by following its character path from the root.\n  let node = root;\n  // A missing edge proves that no dictionary word can equal the query.\n  for (const c of query) { if (!node[c]) return false; node = node[c]; }\n\n  // The full path counts only when it ends at a stored complete word.\n  return node.end === true;\n}\n",
          ts: "function wordExists(words: string[], query: string): boolean {\n  // The root represents the empty prefix shared by every inserted word.\n  const root: any = {};\n\n  // Insert each dictionary word one character at a time.\n  for (const w of words) {\n    // Begin this word at the root of the trie.\n    let node = root;\n    // Reuse an existing character edge or create it, then descend through it.\n    for (const c of w) { node[c] = node[c] || {}; node = node[c]; }\n    // Distinguish a complete word from a prefix such as app inside apple.\n    node.end = true;\n  }\n\n  // Match the query by following its character path from the root.\n  let node = root;\n  // A missing edge proves that no dictionary word can equal the query.\n  for (const c of query) { if (!node[c]) return false; node = node[c]; }\n\n  // The full path counts only when it ends at a stored complete word.\n  return node.end === true;\n}\n",
        },
        time: "O(total chars)",
        space: "O(total chars)",
      },
      {
        ...
        ts: "function wordExists(words: string[], query: string): boolean {\n  return new Set(words).has(query);\n}\n",
        commentedCode: {
          js: "function wordExists(words, query) {\n  // Build a set for exact membership, then ask whether query is one of its words.\n  return new Set(words).has(query);\n}\n",
          ts: "function wordExists(words: string[], query: string): boolean {\n  // Build a set for exact membership, then ask whether query is one of its words.\n  return new Set(words).has(query);\n}\n",
        },
        time: "O(total chars)",
        space: "O(total chars)",
    ...
    solutions: [
      {
        ...
        commentedCode: {
          js: "function countWithPrefix(words, prefix) {\n  // Each trie node counts how many inserted words pass through that prefix.\n  const root = { count: 0 };\n\n  // Insert every word while updating the count after each character transition.\n  for (const w of words) {\n    // Start at the empty-prefix root for this word.\n    let node = root;\n    // Create missing edges, descend, and record that this word uses the new prefix.\n    for (const c of w) { node[c] = node[c] || { count: 0 }; node = node[c]; node.count++; }\n  }\n\n  // Every word starts with the empty prefix; its count is not stored on the root.\n  if (prefix === '') return words.length;\n\n  // Follow the requested prefix through the trie.\n  let node = root;\n  // If any edge is absent, no word starts with the entire prefix.\n  for (const c of prefix) { if (!node[c]) return 0; node = node[c]; }\n\n  // This node's pass-through count is exactly the number of matching words.\n  return node.count;\n}\n",
          ts: "function countWithPrefix(words: string[], prefix: string): number {\n  // Each trie node counts how many inserted words pass through that prefix.\n  const root: any = { count: 0 };\n\n  // Insert every word while updating the count after each character transition.\n  for (const w of words) {\n    // Start at the empty-prefix root for this word.\n    let node = root;\n    // Create missing edges, descend, and record that this word uses the new prefix.\n    for (const c of w) { node[c] = node[c] || { count: 0 }; node = node[c]; node.count++; }\n  }\n\n  // Every word starts with the empty prefix; its count is not stored on the root.\n  if (prefix === '') return words.length;\n\n  // Follow the requested prefix through the trie.\n  let node = root;\n  // If any edge is absent, no word starts with the entire prefix.\n  for (const c of prefix) { if (!node[c]) return 0; node = node[c]; }\n\n  // This node's pass-through count is exactly the number of matching words.\n  return node.count;\n}\n",
        },
        time: "O(total chars)",
        space: "O(total chars)",
      },
      {
        ...
        ts: "function countWithPrefix(words: string[], prefix: string): number {\n  let count = 0;\n  for (const w of words) if (w.startsWith(prefix)) count++;\n  return count;\n}\n",
        commentedCode: {
          js: "function countWithPrefix(words, prefix) {\n  // Accumulate one match for every word whose first characters equal prefix.\n  let count = 0;\n  // startsWith also makes the empty prefix match every word automatically.\n  for (const w of words) if (w.startsWith(prefix)) count++;\n\n  // Return the number of successful prefix comparisons.\n  return count;\n}\n",
          ts: "function countWithPrefix(words: string[], prefix: string): number {\n  // Accumulate one match for every word whose first characters equal prefix.\n  let count = 0;\n  // startsWith also makes the empty prefix match every word automatically.\n  for (const w of words) if (w.startsWith(prefix)) count++;\n\n  // Return the number of successful prefix comparisons.\n  return count;\n}\n",
        },
        time: "O(total chars)",
        space: "O(1)",
    ...
    solutions: [
      {
        ...
        ts: "function longestCommonPrefix(words: string[]): string {\n  if (words.length === 0) return '';\n  const root: any = {};\n  for (const w of words) {\n    let node = root;\n    for (const c of w) { node[c] = node[c] || {}; node = node[c]; }\n    node.end = true;\n  }\n  let prefix = '', node = root;\n  while (true) {\n    const keys = Object.keys(node).filter((k) => k !== 'end');\n    if (keys.length !== 1 || node.end) break;\n    prefix += keys[0];\n    node = node[keys[0]];\n  }\n  return prefix;\n}\n",
        commentedCode: {
          js: "function longestCommonPrefix(words) {\n  // With no words there is no shared prefix to return.\n  if (words.length === 0) return '';\n\n  // Build one trie containing all words so shared prefixes share nodes.\n  const root = {};\n  for (const w of words) {\n    // Insert this word from the empty-prefix root.\n    let node = root;\n    // Reuse shared character edges and create only missing suffix nodes.\n    for (const c of w) { node[c] = node[c] || {}; node = node[c]; }\n    // Mark where this word finishes, even if longer words continue below it.\n    node.end = true;\n  }\n\n  // Grow the answer while the trie has one forced continuation.\n  let prefix = '', node = root;\n  while (true) {\n    // end is metadata, so only character keys represent child branches.\n    const keys = Object.keys(node).filter((k) => k !== 'end');\n    // A branch means words differ; a word end means at least one word stops here.\n    if (keys.length !== 1 || node.end) break;\n    // The sole child character must occur next in every word.\n    prefix += keys[0];\n    // Descend to test the following shared character.\n    node = node[keys[0]];\n  }\n\n  // Everything accumulated before the first stop condition is common to all words.\n  return prefix;\n}\n",
          ts: "function longestCommonPrefix(words: string[]): string {\n  // With no words there is no shared prefix to return.\n  if (words.length === 0) return '';\n\n  // Build one trie containing all words so shared prefixes share nodes.\n  const root: any = {};\n  for (const w of words) {\n    // Insert this word from the empty-prefix root.\n    let node = root;\n    // Reuse shared character edges and create only missing suffix nodes.\n    for (const c of w) { node[c] = node[c] || {}; node = node[c]; }\n    // Mark where this word finishes, even if longer words continue below it.\n    node.end = true;\n  }\n\n  // Grow the answer while the trie has one forced continuation.\n  let prefix = '', node = root;\n  while (true) {\n    // end is metadata, so only character keys represent child branches.\n    const keys = Object.keys(node).filter((k) => k !== 'end');\n    // A branch means words differ; a word end means at least one word stops here.\n    if (keys.length !== 1 || node.end) break;\n    // The sole child character must occur next in every word.\n    prefix += keys[0];\n    // Descend to test the following shared character.\n    node = node[keys[0]];\n  }\n\n  // Everything accumulated before the first stop condition is common to all words.\n  return prefix;\n}\n",
        },
        time: "O(total chars)",
        space: "O(total chars)",
      },
      {
        ...
        ts: "function longestCommonPrefix(words: string[]): string {\n  if (words.length === 0) return '';\n  let prefix = words[0];\n  for (const w of words) {\n    let i = 0;\n    while (i < prefix.length && i < w.length && prefix[i] === w[i]) i++;\n    prefix = prefix.slice(0, i);\n  }\n  return prefix;\n}\n",
        commentedCode: {
          js: "function longestCommonPrefix(words) {\n  // An empty collection has no common characters.\n  if (words.length === 0) return '';\n\n  // The first word is the longest possible candidate prefix.\n  let prefix = words[0];\n  for (const w of words) {\n    // Count matching columns until either string ends or characters differ.\n    let i = 0;\n    while (i < prefix.length && i < w.length && prefix[i] === w[i]) i++;\n    // Discard every candidate character not shared with the current word.\n    prefix = prefix.slice(0, i);\n  }\n\n  // Repeated shrinking leaves exactly the prefix shared by all words.\n  return prefix;\n}\n",
          ts: "function longestCommonPrefix(words: string[]): string {\n  // An empty collection has no common characters.\n  if (words.length === 0) return '';\n\n  // The first word is the longest possible candidate prefix.\n  let prefix = words[0];\n  for (const w of words) {\n    // Count matching columns until either string ends or characters differ.\n    let i = 0;\n    while (i < prefix.length && i < w.length && prefix[i] === w[i]) i++;\n    // Discard every candidate character not shared with the current word.\n    prefix = prefix.slice(0, i);\n  }\n\n  // Repeated shrinking leaves exactly the prefix shared by all words.\n  return prefix;\n}\n",
        },
        time: "O(total chars)",
        space: "O(1)",
    ...
    solutions: [
      {
        ...
        commentedCode: {
          js: "function wildcardSearch(words, query) {\n  // Store all dictionary words in a trie for shared-prefix searching.\n  const root = {};\n  for (const w of words) {\n    // Insert this word from the root.\n    let node = root;\n    // Follow existing character edges or create missing ones.\n    for (const c of w) { node[c] = node[c] || {}; node = node[c]; }\n    // A match must finish at a complete word, not merely a prefix.\n    node.end = true;\n  }\n\n  // Search from trie node using query position i.\n  const dfs = (node, i) => {\n    // Once every query character is consumed, accept only a word-ending node.\n    if (i === query.length) return node.end === true;\n    // Inspect the next literal character or wildcard.\n    const c = query[i];\n    if (c === '.') {\n      // A wildcard consumes exactly one character, so try every character child.\n      for (const k of Object.keys(node)) if (k !== 'end' && dfs(node[k], i + 1)) return true;\n      // No child path matched the remaining query.\n      return false;\n    }\n    // A literal can continue only through the edge carrying that exact character.\n    return node[c] ? dfs(node[c], i + 1) : false;\n  };\n\n  // Start matching at the empty prefix and the first query position.\n  return dfs(root, 0);\n}\n",
          ts: "function wildcardSearch(words: string[], query: string): boolean {\n  // Store all dictionary words in a trie for shared-prefix searching.\n  const root: any = {};\n  for (const w of words) {\n    // Insert this word from the root.\n    let node = root;\n    // Follow existing character edges or create missing ones.\n    for (const c of w) { node[c] = node[c] || {}; node = node[c]; }\n    // A match must finish at a complete word, not merely a prefix.\n    node.end = true;\n  }\n\n  // Search from trie node using query position i.\n  const dfs = (node: any, i: number): boolean => {\n    // Once every query character is consumed, accept only a word-ending node.\n    if (i === query.length) return node.end === true;\n    // Inspect the next literal character or wildcard.\n    const c = query[i];\n    if (c === '.') {\n      // A wildcard consumes exactly one character, so try every character child.\n      for (const k of Object.keys(node)) if (k !== 'end' && dfs(node[k], i + 1)) return true;\n      // No child path matched the remaining query.\n      return false;\n    }\n    // A literal can continue only through the edge carrying that exact character.\n    return node[c] ? dfs(node[c], i + 1) : false;\n  };\n\n  // Start matching at the empty prefix and the first query position.\n  return dfs(root, 0);\n}\n",
        },
        time: "O(words · length)",
        space: "O(total chars)",
      },
      {
        ...
        ts: "function wildcardSearch(words: string[], query: string): boolean {\n  for (const w of words) {\n    if (w.length !== query.length) continue;\n    let ok = true;\n    for (let i = 0; i < query.length; i++) if (query[i] !== '.' && query[i] !== w[i]) { ok = false; break; }\n    if (ok) return true;\n  }\n  return false;\n}\n",
        commentedCode: {
          js: "function wildcardSearch(words, query) {\n  // Test each dictionary word as a possible full-query match.\n  for (const w of words) {\n    // Since each wildcard represents one character, lengths must be equal.\n    if (w.length !== query.length) continue;\n    // Assume a match until a literal query character disagrees.\n    let ok = true;\n    // Wildcards accept any character; literals must equal the word at this position.\n    for (let i = 0; i < query.length; i++) if (query[i] !== '.' && query[i] !== w[i]) { ok = false; break; }\n    // One complete positional match is enough to answer true.\n    if (ok) return true;\n  }\n\n  // Every candidate had the wrong length or a conflicting literal.\n  return false;\n}\n",
          ts: "function wildcardSearch(words: string[], query: string): boolean {\n  // Test each dictionary word as a possible full-query match.\n  for (const w of words) {\n    // Since each wildcard represents one character, lengths must be equal.\n    if (w.length !== query.length) continue;\n    // Assume a match until a literal query character disagrees.\n    let ok = true;\n    // Wildcards accept any character; literals must equal the word at this position.\n    for (let i = 0; i < query.length; i++) if (query[i] !== '.' && query[i] !== w[i]) { ok = false; break; }\n    // One complete positional match is enough to answer true.\n    if (ok) return true;\n  }\n\n  // Every candidate had the wrong length or a conflicting literal.\n  return false;\n}\n",
        },
        time: "O(words · length)",
        space: "O(1)",
    ...
    solutions: [
      {
        ...
        commentedCode: {
          js: "function replaceWords(dict, sentence) {\n  // Build a trie whose terminal nodes mark valid dictionary roots.\n  const root = {};\n  for (const r of dict) {\n    // Insert each root from the shared empty-prefix node.\n    let node = root;\n    // Reuse common root prefixes and create only missing character edges.\n    for (const c of r) { node[c] = node[c] || {}; node = node[c]; }\n    // This node completes a root that may replace a longer sentence word.\n    node.end = true;\n  }\n\n  // Find the shortest dictionary root that prefixes one sentence word.\n  const shorten = (word) => {\n    // Traverse the trie while building the matched prefix.\n    let node = root, pre = '';\n    for (const c of word) {\n      // Falling off the trie means no dictionary root can match this word.\n      if (!node[c]) return word;\n      // Consume the next character and move to its trie state.\n      pre += c; node = node[c];\n      // The first terminal reached is necessarily the shortest matching root.\n      if (node.end) return pre;\n    }\n    // No earlier terminal root was found, so preserve the original word.\n    return word;\n  };\n\n  // Transform each space-delimited word and rebuild the sentence.\n  return sentence.split(' ').map(shorten).join(' ');\n}\n",
          ts: "function replaceWords(dict: string[], sentence: string): string {\n  // Build a trie whose terminal nodes mark valid dictionary roots.\n  const root: any = {};\n  for (const r of dict) {\n    // Insert each root from the shared empty-prefix node.\n    let node = root;\n    // Reuse common root prefixes and create only missing character edges.\n    for (const c of r) { node[c] = node[c] || {}; node = node[c]; }\n    // This node completes a root that may replace a longer sentence word.\n    node.end = true;\n  }\n\n  // Find the shortest dictionary root that prefixes one sentence word.\n  const shorten = (word: string): string => {\n    // Traverse the trie while building the matched prefix.\n    let node = root, pre = '';\n    for (const c of word) {\n      // Falling off the trie means no dictionary root can match this word.\n      if (!node[c]) return word;\n      // Consume the next character and move to its trie state.\n      pre += c; node = node[c];\n      // The first terminal reached is necessarily the shortest matching root.\n      if (node.end) return pre;\n    }\n    // No earlier terminal root was found, so preserve the original word.\n    return word;\n  };\n\n  // Transform each space-delimited word and rebuild the sentence.\n  return sentence.split(' ').map(shorten).join(' ');\n}\n",
        },
      ...
      },
      {
        ...
        ts: "function replaceWords(dict: string[], sentence: string): string {\n  const roots = dict.slice().sort((a, b) => a.length - b.length);\n  return sentence.split(' ').map((word) => {\n    for (const r of roots) if (word.startsWith(r)) return r;\n    return word;\n  }).join(' ');\n}\n",
        commentedCode: {
          js: "function replaceWords(dict, sentence) {\n  // Copy before sorting so the caller's dictionary is not mutated; shortest roots come first.\n  const roots = dict.slice().sort((a, b) => a.length - b.length);\n\n  // Replace each sentence word independently, then restore the spaces.\n  return sentence.split(' ').map((word) => {\n    // The first prefix match is shortest because roots are ordered by length.\n    for (const r of roots) if (word.startsWith(r)) return r;\n    // Keep a word unchanged when no dictionary root prefixes it.\n    return word;\n  }).join(' ');\n}\n",
          ts: "function replaceWords(dict: string[], sentence: string): string {\n  // Copy before sorting so the caller's dictionary is not mutated; shortest roots come first.\n  const roots = dict.slice().sort((a, b) => a.length - b.length);\n\n  // Replace each sentence word independently, then restore the spaces.\n  return sentence.split(' ').map((word) => {\n    // The first prefix match is shortest because roots are ordered by length.\n    for (const r of roots) if (word.startsWith(r)) return r;\n    // Keep a word unchanged when no dictionary root prefixes it.\n    return word;\n  }).join(' ');\n}\n",
        },
    ...
    solutions: [
      {
        ...
        ts: "function longestBuildableWord(words: string[]): string {\n  const root: any = {};\n  for (const w of words) {\n    let node = root;\n    for (const c of w) { node[c] = node[c] || {}; node = node[c]; }\n    node.end = true;\n  }\n  let best = '';\n  for (const w of words) {\n    let node = root, ok = true;\n    for (const c of w) { node = node[c]; if (!node.end) { ok = false; break; } }\n    if (ok && (w.length > best.length || (w.length === best.length && w < best))) best = w;\n  }\n  return best;\n}\n",
        commentedCode: {
          js: "function longestBuildableWord(words) {\n  // Build a trie that records which character paths are complete input words.\n  const root = {};\n  for (const w of words) {\n    // Insert this word from the empty prefix.\n    let node = root;\n    // Share nodes among equal prefixes and create the rest of the path.\n    for (const c of w) { node[c] = node[c] || {}; node = node[c]; }\n    // Mark this exact prefix length as available for building longer words.\n    node.end = true;\n  }\n\n  // The empty string is the fallback if no word has every required prefix.\n  let best = '';\n  for (const w of words) {\n    // Verify this candidate by walking all of its progressively longer prefixes.\n    let node = root, ok = true;\n    // Every reached node must terminate an input word; otherwise the chain is broken.\n    for (const c of w) { node = node[c]; if (!node.end) { ok = false; break; } }\n    // Prefer a longer valid chain, then the lexicographically smaller tie.\n    if (ok && (w.length > best.length || (w.length === best.length && w < best))) best = w;\n  }\n\n  // Return the strongest candidate found under both ordering rules.\n  return best;\n}\n",
          ts: "function longestBuildableWord(words: string[]): string {\n  // Build a trie that records which character paths are complete input words.\n  const root: any = {};\n  for (const w of words) {\n    // Insert this word from the empty prefix.\n    let node = root;\n    // Share nodes among equal prefixes and create the rest of the path.\n    for (const c of w) { node[c] = node[c] || {}; node = node[c]; }\n    // Mark this exact prefix length as available for building longer words.\n    node.end = true;\n  }\n\n  // The empty string is the fallback if no word has every required prefix.\n  let best = '';\n  for (const w of words) {\n    // Verify this candidate by walking all of its progressively longer prefixes.\n    let node = root, ok = true;\n    // Every reached node must terminate an input word; otherwise the chain is broken.\n    for (const c of w) { node = node[c]; if (!node.end) { ok = false; break; } }\n    // Prefer a longer valid chain, then the lexicographically smaller tie.\n    if (ok && (w.length > best.length || (w.length === best.length && w < best))) best = w;\n  }\n\n  // Return the strongest candidate found under both ordering rules.\n  return best;\n}\n",
        },
        time: "O(total chars)",
        space: "O(total chars)",
      },
      {
        ...
        ts: "function longestBuildableWord(words: string[]): string {\n  const set = new Set(words);\n  let best = '';\n  for (const w of words) {\n    let ok = true;\n    for (let i = 1; i < w.length; i++) if (!set.has(w.slice(0, i))) { ok = false; break; }\n    if (ok && (w.length > best.length || (w.length === best.length && w < best))) best = w;\n  }\n  return best;\n}\n",
        commentedCode: {
          js: "function longestBuildableWord(words) {\n  // Exact set membership lets each candidate prefix be checked directly.\n  const set = new Set(words);\n  // Keep the best valid word seen so far.\n  let best = '';\n  for (const w of words) {\n    // Reject the candidate if any proper non-empty prefix is absent.\n    let ok = true;\n    // The full word itself is already in the input, so only shorter prefixes need checks.\n    for (let i = 1; i < w.length; i++) if (!set.has(w.slice(0, i))) { ok = false; break; }\n    // Prefer greater length and use lexicographic order only to break a tie.\n    if (ok && (w.length > best.length || (w.length === best.length && w < best))) best = w;\n  }\n\n  // Return an empty string only when every candidate lacked a required prefix.\n  return best;\n}\n",
          ts: "function longestBuildableWord(words: string[]): string {\n  // Exact set membership lets each candidate prefix be checked directly.\n  const set = new Set(words);\n  // Keep the best valid word seen so far.\n  let best = '';\n  for (const w of words) {\n    // Reject the candidate if any proper non-empty prefix is absent.\n    let ok = true;\n    // The full word itself is already in the input, so only shorter prefixes need checks.\n    for (let i = 1; i < w.length; i++) if (!set.has(w.slice(0, i))) { ok = false; break; }\n    // Prefer greater length and use lexicographic order only to break a tie.\n    if (ok && (w.length > best.length || (w.length === best.length && w < best))) best = w;\n  }\n\n  // Return an empty string only when every candidate lacked a required prefix.\n  return best;\n}\n",
        },
        time: "O(total chars²)",
        space: "O(total chars)",
    ...
    solutions: [
      {
        ...
        commentedCode: {
          js: "function autocomplete(words, prefix) {\n  // Store all words in a trie so completions occupy one prefix subtree.\n  const root = {};\n  for (const w of words) {\n    // Begin inserting this word at the empty prefix.\n    let node = root;\n    // Reuse existing shared-prefix edges and create missing characters.\n    for (const c of w) { node[c] = node[c] || {}; node = node[c]; }\n    // Mark the exact word so it can be emitted during subtree traversal.\n    node.end = true;\n  }\n\n  // Descend to the trie node representing the requested prefix.\n  let node = root;\n  // A missing prefix edge means there are no possible completions.\n  for (const c of prefix) { if (!node[c]) return []; node = node[c]; }\n\n  // Collect complete words reachable below the prefix node.\n  const out = [];\n  // path stores only the characters added after the fixed prefix.\n  const dfs = (n, path) => {\n    // Reaching an end marker completes prefix + path as a dictionary word.\n    if (n.end) out.push(prefix + path);\n    // Explore every character child; end is metadata rather than an edge.\n    for (const k of Object.keys(n)) if (k !== 'end') dfs(n[k], path + k);\n  };\n  // An empty path also allows the prefix itself to be returned when it is a word.\n  dfs(node, '');\n\n  // Normalize traversal order into the required lexicographic order.\n  return out.sort();\n}\n",
          ts: "function autocomplete(words: string[], prefix: string): string[] {\n  // Store all words in a trie so completions occupy one prefix subtree.\n  const root: any = {};\n  for (const w of words) {\n    // Begin inserting this word at the empty prefix.\n    let node = root;\n    // Reuse existing shared-prefix edges and create missing characters.\n    for (const c of w) { node[c] = node[c] || {}; node = node[c]; }\n    // Mark the exact word so it can be emitted during subtree traversal.\n    node.end = true;\n  }\n\n  // Descend to the trie node representing the requested prefix.\n  let node = root;\n  // A missing prefix edge means there are no possible completions.\n  for (const c of prefix) { if (!node[c]) return []; node = node[c]; }\n\n  // Collect complete words reachable below the prefix node.\n  const out: string[] = [];\n  // path stores only the characters added after the fixed prefix.\n  const dfs = (n: any, path: string): void => {\n    // Reaching an end marker completes prefix + path as a dictionary word.\n    if (n.end) out.push(prefix + path);\n    // Explore every character child; end is metadata rather than an edge.\n    for (const k of Object.keys(n)) if (k !== 'end') dfs(n[k], path + k);\n  };\n  // An empty path also allows the prefix itself to be returned when it is a word.\n  dfs(node, '');\n\n  // Normalize traversal order into the required lexicographic order.\n  return out.sort();\n}\n",
        },
      ...
      },
      {
        ...
        ts: "function autocomplete(words: string[], prefix: string): string[] {\n  return words.filter((w) => w.startsWith(prefix)).sort();\n}\n",
        commentedCode: {
          js: "function autocomplete(words, prefix) {\n  // Keep exactly the words beginning with prefix, then order those matches lexicographically.\n  return words.filter((w) => w.startsWith(prefix)).sort();\n}\n",
          ts: "function autocomplete(words: string[], prefix: string): string[] {\n  // Keep exactly the words beginning with prefix, then order those matches lexicographically.\n  return words.filter((w) => w.startsWith(prefix)).sort();\n}\n",
        },
        time: "O(total chars + m log m)",
        space: "O(m)",
    ...
    solutions: [
      {
        ...
        ts: "function wordBreak(s: string, dict: string[]): boolean {\n  const root: any = {};\n  for (const w of dict) {\n    let node = root;\n    for (const c of w) { node[c] = node[c] || {}; node = node[c]; }\n    node.end = true;\n  }\n  const n = s.length;\n  const dp = new Array(n + 1).fill(false);\n  dp[0] = true;\n  for (let i = 0; i < n; i++) {\n    if (!dp[i]) continue;\n    let node = root;\n    for (let j = i; j < n; j++) {\n      node = node[s[j]];\n      if (!node) break;\n      if (node.end) dp[j + 1] = true;\n    }\n  }\n  return dp[n];\n}\n",
        commentedCode: {
          js: "function wordBreak(s, dict) {\n  // Build a trie so all dictionary words beginning at one index share a walk.\n  const root = {};\n  for (const w of dict) {\n    // Insert each reusable dictionary word from the root.\n    let node = root;\n    // Follow or create the path for every character in the word.\n    for (const c of w) { node[c] = node[c] || {}; node = node[c]; }\n    // This node marks a legal segment boundary.\n    node.end = true;\n  }\n\n  // dp[i] says whether the prefix s[0..i) can be fully segmented.\n  const n = s.length;\n  const dp = new Array(n + 1).fill(false);\n  // The empty prefix is the starting reachable boundary.\n  dp[0] = true;\n\n  // Try to start a dictionary word at every reachable boundary.\n  for (let i = 0; i < n; i++) {\n    // An unreachable boundary cannot extend a valid segmentation.\n    if (!dp[i]) continue;\n    // Each new start position begins its trie search at the root.\n    let node = root;\n    for (let j = i; j < n; j++) {\n      // Consume s[j] by following the matching trie edge.\n      node = node[s[j]];\n      // Once the path fails, no longer substring from i can be a dictionary word.\n      if (!node) break;\n      // A terminal node makes the boundary after j reachable for future segments.\n      if (node.end) dp[j + 1] = true;\n    }\n  }\n\n  // The entire string is segmentable exactly when its final boundary is reachable.\n  return dp[n];\n}\n",
          ts: "function wordBreak(s: string, dict: string[]): boolean {\n  // Build a trie so all dictionary words beginning at one index share a walk.\n  const root: any = {};\n  for (const w of dict) {\n    // Insert each reusable dictionary word from the root.\n    let node = root;\n    // Follow or create the path for every character in the word.\n    for (const c of w) { node[c] = node[c] || {}; node = node[c]; }\n    // This node marks a legal segment boundary.\n    node.end = true;\n  }\n\n  // dp[i] says whether the prefix s[0..i) can be fully segmented.\n  const n = s.length;\n  const dp = new Array(n + 1).fill(false);\n  // The empty prefix is the starting reachable boundary.\n  dp[0] = true;\n\n  // Try to start a dictionary word at every reachable boundary.\n  for (let i = 0; i < n; i++) {\n    // An unreachable boundary cannot extend a valid segmentation.\n    if (!dp[i]) continue;\n    // Each new start position begins its trie search at the root.\n    let node = root;\n    for (let j = i; j < n; j++) {\n      // Consume s[j] by following the matching trie edge.\n      node = node[s[j]];\n      // Once the path fails, no longer substring from i can be a dictionary word.\n      if (!node) break;\n      // A terminal node makes the boundary after j reachable for future segments.\n      if (node.end) dp[j + 1] = true;\n    }\n  }\n\n  // The entire string is segmentable exactly when its final boundary is reachable.\n  return dp[n];\n}\n",
        },
        time: "O(n²)",
        space: "O(n + dict)",
      },
      {
        ...
        ts: "function wordBreak(s: string, dict: string[]): boolean {\n  const set = new Set(dict);\n  const n = s.length;\n  const dp = new Array(n + 1).fill(false);\n  dp[0] = true;\n  for (let i = 1; i <= n; i++)\n    for (let j = 0; j < i; j++)\n      if (dp[j] && set.has(s.slice(j, i))) { dp[i] = true; break; }\n  return dp[n];\n}\n",
        commentedCode: {
          js: "function wordBreak(s, dict) {\n  // Use constant-time membership checks for candidate dictionary segments.\n  const set = new Set(dict);\n  // dp[i] represents whether s[0..i) has a valid segmentation.\n  const n = s.length;\n  const dp = new Array(n + 1).fill(false);\n  // No words are needed to segment the empty prefix.\n  dp[0] = true;\n\n  // Compute reachability for each ending boundary from left to right.\n  for (let i = 1; i <= n; i++)\n    // Try every earlier reachable boundary as the start of the final word.\n    for (let j = 0; j < i; j++)\n      // A reachable prefix plus a dictionary substring makes boundary i reachable.\n      if (dp[j] && set.has(s.slice(j, i))) { dp[i] = true; break; }\n\n  // The last boundary answers whether all characters can be segmented.\n  return dp[n];\n}\n",
          ts: "function wordBreak(s: string, dict: string[]): boolean {\n  // Use constant-time membership checks for candidate dictionary segments.\n  const set = new Set(dict);\n  // dp[i] represents whether s[0..i) has a valid segmentation.\n  const n = s.length;\n  const dp = new Array(n + 1).fill(false);\n  // No words are needed to segment the empty prefix.\n  dp[0] = true;\n\n  // Compute reachability for each ending boundary from left to right.\n  for (let i = 1; i <= n; i++)\n    // Try every earlier reachable boundary as the start of the final word.\n    for (let j = 0; j < i; j++)\n      // A reachable prefix plus a dictionary substring makes boundary i reachable.\n      if (dp[j] && set.has(s.slice(j, i))) { dp[i] = true; break; }\n\n  // The last boundary answers whether all characters can be segmented.\n  return dp[n];\n}\n",
        },
        time: "O(n²)",
        space: "O(n + dict)",
    ...
    solutions: [
      {
        ...
        commentedCode: {
          js: "function maximumXor(nums) {\n  // A pair needs two distinct array positions; otherwise no XOR pair exists.\n  if (nums.length < 2) return 0;\n\n  // Read every 32-bit position from most significant to least significant.\n  const HIGH = 31, root = {};\n  for (const num of nums) {\n    // Insert this number as one root-to-leaf sequence of binary digits.\n    let node = root;\n    // Extract each bit, create its 0/1 branch if needed, and descend.\n    for (let b = HIGH; b >= 0; b--) { const bit = (num >> b) & 1; node[bit] = node[bit] || {}; node = node[bit]; }\n  }\n\n  // Track the largest XOR value produced against any stored number.\n  let best = 0;\n  for (const num of nums) {\n    // cur accumulates the XOR bits chosen during this greedy trie walk.\n    let node = root, cur = 0;\n    for (let b = HIGH; b >= 0; b--) {\n      // The opposite input bit would make XOR bit b equal to 1.\n      const bit = (num >> b) & 1, want = bit ^ 1;\n      // Prefer that opposite branch; otherwise the equal-bit branch is forced.\n      if (node[want]) { cur |= (1 << b); node = node[want]; } else node = node[bit];\n    }\n    // Keep the strongest greedy match found for any input number.\n    best = Math.max(best, cur);\n  }\n\n  // Most-significant-bit-first greedy choices maximize the numeric XOR value.\n  return best;\n}\n",
          ts: "function maximumXor(nums: number[]): number {\n  // A pair needs two distinct array positions; otherwise no XOR pair exists.\n  if (nums.length < 2) return 0;\n\n  // Read every 32-bit position from most significant to least significant.\n  const HIGH = 31, root: any = {};\n  for (const num of nums) {\n    // Insert this number as one root-to-leaf sequence of binary digits.\n    let node = root;\n    // Extract each bit, create its 0/1 branch if needed, and descend.\n    for (let b = HIGH; b >= 0; b--) { const bit = (num >> b) & 1; node[bit] = node[bit] || {}; node = node[bit]; }\n  }\n\n  // Track the largest XOR value produced against any stored number.\n  let best = 0;\n  for (const num of nums) {\n    // cur accumulates the XOR bits chosen during this greedy trie walk.\n    let node = root, cur = 0;\n    for (let b = HIGH; b >= 0; b--) {\n      // The opposite input bit would make XOR bit b equal to 1.\n      const bit = (num >> b) & 1, want = bit ^ 1;\n      // Prefer that opposite branch; otherwise the equal-bit branch is forced.\n      if (node[want]) { cur |= (1 << b); node = node[want]; } else node = node[bit];\n    }\n    // Keep the strongest greedy match found for any input number.\n    best = Math.max(best, cur);\n  }\n\n  // Most-significant-bit-first greedy choices maximize the numeric XOR value.\n  return best;\n}\n",
        },
        time: "O(n · 32)",
        space: "O(n · 32)",
      },
      {
        ...
        ts: "function maximumXor(nums: number[]): number {\n  let best = 0;\n  for (let i = 0; i < nums.length; i++)\n    for (let j = i + 1; j < nums.length; j++) best = Math.max(best, nums[i] ^ nums[j]);\n  return best;\n}\n",
        commentedCode: {
          js: "function maximumXor(nums) {\n  // Zero is the correct fallback when fewer than two elements form a pair.\n  let best = 0;\n  // Choose the first index of every unordered pair.\n  for (let i = 0; i < nums.length; i++)\n    // Start after i so indices are distinct and no pair is evaluated twice.\n    for (let j = i + 1; j < nums.length; j++) best = Math.max(best, nums[i] ^ nums[j]);\n\n  // Return the greatest pairwise XOR encountered.\n  return best;\n}\n",
          ts: "function maximumXor(nums: number[]): number {\n  // Zero is the correct fallback when fewer than two elements form a pair.\n  let best = 0;\n  // Choose the first index of every unordered pair.\n  for (let i = 0; i < nums.length; i++)\n    // Start after i so indices are distinct and no pair is evaluated twice.\n    for (let j = i + 1; j < nums.length; j++) best = Math.max(best, nums[i] ^ nums[j]);\n\n  // Return the greatest pairwise XOR encountered.\n  return best;\n}\n",
        },
        time: "O(n²)",
        space: "O(1)",
    ...
    solutions: [
      {
        ...
        ts: "function countDistinctSubstrings(s: string): number {\n  const root: any = {};\n  let count = 0;\n  for (let i = 0; i < s.length; i++) {\n    let node = root;\n    for (let j = i; j < s.length; j++) {\n      const c = s[j];\n      if (!node[c]) { node[c] = {}; count++; }\n      node = node[c];\n    }\n  }\n  return count;\n}\n",
        commentedCode: {
          js: "function countDistinctSubstrings(s) {\n  // In a suffix trie, every non-root node represents one distinct substring.\n  const root = {};\n  // Count nodes at the moment they are first created.\n  let count = 0;\n\n  // Start a trie insertion at every possible substring beginning.\n  for (let i = 0; i < s.length; i++) {\n    // Each suffix begins again at the empty-string root.\n    let node = root;\n    for (let j = i; j < s.length; j++) {\n      // Extending the current path by s[j] forms substring s[i..j].\n      const c = s[j];\n      // A missing edge means this exact substring path has never appeared before.\n      if (!node[c]) { node[c] = {}; count++; }\n      // Descend so the next character extends the same starting substring.\n      node = node[c];\n    }\n  }\n\n  // Reused paths represent duplicates, so the number of created nodes is distinct count.\n  return count;\n}\n",
          ts: "function countDistinctSubstrings(s: string): number {\n  // In a suffix trie, every non-root node represents one distinct substring.\n  const root: any = {};\n  // Count nodes at the moment they are first created.\n  let count = 0;\n\n  // Start a trie insertion at every possible substring beginning.\n  for (let i = 0; i < s.length; i++) {\n    // Each suffix begins again at the empty-string root.\n    let node = root;\n    for (let j = i; j < s.length; j++) {\n      // Extending the current path by s[j] forms substring s[i..j].\n      const c = s[j];\n      // A missing edge means this exact substring path has never appeared before.\n      if (!node[c]) { node[c] = {}; count++; }\n      // Descend so the next character extends the same starting substring.\n      node = node[c];\n    }\n  }\n\n  // Reused paths represent duplicates, so the number of created nodes is distinct count.\n  return count;\n}\n",
        },
        time: "O(n²)",
        space: "O(n²)",
      },
      {
        ...
        commentedCode: {
          js: "function countDistinctSubstrings(s) {\n  // A set automatically collapses repeated substring values.\n  const set = new Set();\n  // Choose every inclusive substring start index.\n  for (let i = 0; i < s.length; i++)\n    // Choose each exclusive end after i and insert the corresponding non-empty slice.\n    for (let j = i + 1; j <= s.length; j++) set.add(s.slice(i, j));\n\n  // Each remaining set entry is one distinct non-empty substring.\n  return set.size;\n}\n",
          ts: "function countDistinctSubstrings(s: string): number {\n  // A set automatically collapses repeated substring values.\n  const set = new Set<string>();\n  // Choose every inclusive substring start index.\n  for (let i = 0; i < s.length; i++)\n    // Choose each exclusive end after i and insert the corresponding non-empty slice.\n    for (let j = i + 1; j <= s.length; j++) set.add(s.slice(i, j));\n\n  // Each remaining set entry is one distinct non-empty substring.\n  return set.size;\n}\n",
        },
        time: "O(n³)",
        space: "O(n²)",
